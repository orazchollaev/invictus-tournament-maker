export default {
  drawCeremony: {
    potLabel: "Torba {n}",
    potSingle: "Torba",
    byeSlot: "BAY {n}",
    matchSlot: "Maç {n}",
    wildcardsPot: "Joker Takımlar",
    groupWinnersPot: "Grup Birincileri",
    runnersUpPot: "Grup İkincileri",
    rankPot: "{n}. Sıra",
    title: "Kura Çekimi",
    potsHint: "Takımları torbalar arasında sürükleyin, ardından kurayı başlatın.",
    crossLocked: "Çapraz kura — eşleşmeler sabittir. Göstermek için kurayı başlatın.",
    resetPots: "Torbaları sıfırla",
    startDraw: "Kurayı Başlat",
    useOldDraw: "Eski Kurayı Kullan",
    manageTeams: "Takımlar",
    skipDraw: "Kurayı Atla",
    pauseDraw: "Duraklat",
    resumeDraw: "Devam Et",
    continue: "Devam Et",
    speed: "Hız",
    speedNormal: "Normal",
    speedFast: "Hızlı",
    settingsLabel: "Kura Çekimi",
    settingsDesc: "Takımlar gruplara veya eşleşmelere çekilirken animasyonlu kura göster",
    errors: {
      unassigned: "Tüm takımlar bir torbaya yerleştirilmeli.",
      duplicate: "Bir takım birden fazla torbada görünüyor.",
      emptyPot: "Torbalar boş olamaz.",
    },
    playoffDrawTitle: "Play-off Kura Çekimi",
  },
  matchScore: {
    penalties: "Penaltı atışları",
    penaltiesHint: "Eşitlik bozulmadı — turu penaltılar belirler.",
    penaltiesTied: "Penaltı atışları berabere bitemez.",
    simulate: "Simüle et",
    clear: "Sonucu temizle",
  },
  bracket: {
    fitScreen: "Ekrana sığdır",
    zoomHint: "Yakınlaştırmak için sıkıştırın · sıfırlamak için çift dokunun",
    legPick: "Hangi maçı girmek istiyorsun?",
  },
  nav: {
    tournaments: "Turnuvalar",
    teams: "Takımlar",
    players: "Oyuncular",
    history: "Geçmiş",
    settings: "Ayarlar",
  },
  guide: {
    title: "Nasıl Kullanılır",
    intro:
      "Burada yeni misiniz? İlk turnuvanızı oluşturmak ve simüle etmek için bu adımları takip edin.",
    steps: {
      teams: {
        title: "Takımlarınızı ekleyin",
        desc: "Takımlar sekmesini açın ve isim, kısaltma, güç reytingi ve renk belirlemek için “+ Takım Ekle” seçeneğine dokunun. Aceleniz mi var? Hazır bir takım listesi yüklemek için Ayarlar → Örnek Veri bölümüne gidin.",
      },
      players: {
        title: "Oyuncu ekleyin (opsiyonel)",
        desc: "Bir takımın sayfasını açıp kadrosunu oluşturun — her oyuncu için isim, mevki ve güç girin. Maç sonuçlarına kadro gücü de yansısın istiyorsanız Ayarlar → Simülasyon'dan “Oyuncu Gücünü Kullan”ı açın.",
      },
      create: {
        title: "Bir turnuva oluşturun",
        desc: "Turnuvalar sekmesinde “+ Yeni Turnuva” seçeneğine dokunun, turnuvaya bir isim verin ve takımları seçin. Bir format seçin (Eleme Usulü Braket, Gruplar + Eleme veya Lig) ve ardından grup sayısı, maç başına ayak sayısı (tek maç veya iç saha & dış saha) ve galibiyet/beraberlik puanları gibi seçenekleri ayarlayın.",
      },
      play: {
        title: "Maçları oynayın veya simüle edin",
        desc: "Skorları “Sonucu Ayarla” ile kendiniz girin veya her şeyi anında oynamak için “Tümünü Simüle Et” seçeneğine basın. Gruplar, Braket, Lig, İstatistikler ve Fikstür sekmelerine göz atın ve grup aşaması bittiğinde “Elemelere Geç” seçeneğini kullanın.",
      },
      simulate: {
        title: "Monte Carlo simülasyonu çalıştırın",
        desc: "Bir turnuvanın Ayarlarını açın ve binlerce kez oynatmak için “10.000× Simüle Et” seçeneğine dokunun. Simülasyon Sonuçları sayfası, her takımın şampiyonluk oranlarını ve ortalama istatistiklerini gösterir.",
      },
      seasons: {
        title: "Yeni sezonlar başlatın ve geçmişi takip edin",
        desc: "Bir turnuvayı aynı takımlarla yeniden oynamak için “Yeni Sezon” seçeneğine dokunun. Geçmiş sekmesi, her sezondaki Şampiyonlarınızı, Tüm Zamanların Puan Durumunu ve İstatistiklerini bir araya getirir.",
      },
      settings: {
        title: "Ayarlar'da özelleştirin",
        desc: "Dili değiştirin, temalar arasında geçiş yapın (Açık, Koyu veya Dünya Kupası 2026), tablo kurallarını ve görüntüleme seçeneklerini ince ayarlayın ve tüm verilerinizi Ayarlar sekmesinden yedekleyin veya dışa aktarın.",
      },
    },
    outro:
      "İşte bu kadar — daha sonra bir turnuvanın Ayarlar sayfasından neredeyse her şeyi değiştirebilirsiniz.",
  },
  common: {
    next: "İleri",
    previousPage: "Önceki sayfa",
    nextPage: "Sonraki sayfa",
    pageOf: "{page}/{total}",
    remove: "Kaldır",
    simulate: "Simüle Et",
    info: "Bilgi",
    pickColor: "Renk seçiciyi aç",
    back: "Geri",
    close: "Kapat",
    cancel: "İptal",
    optional: "İsteğe bağlı",
    save: "Kaydet",
    delete: "Sil",
    edit: "Düzenle",
    open: "Aç",
    export: "Dışa Aktar",
    import: "İçe Aktar",
    reset: "Sıfırla",
    load: "Yükle",
    on: "Açık",
    off: "Kapalı",
    show: "Göster",
    hide: "Gizle",
    single: "Tek",
    double: "Çift",
    triple: "3×",
    quadruple: "4×",
    half: "0.5×",
    random: "Rastgele",
    seeded: "Sıralı",
    manual: "Manuel",
    search: "Ara",
    filter: "Filtrele",
    create: "Oluştur",
    new: "Yeni",
    noMatch: '"{query}" ile eşleşen sonuç yok.',
    season: "sezon | sezon",
    teams: "{n} takım",
    team: "Takım",
    update: "Güncelle",
    newVersionAvailable: "Yeni sürüm mevcut",
    starOnGithub: "GitHub'da Yıldız",
    star: "Yıldız",
    setResult: "Sonuç Gir",
    moveUp: "Yukarı taşı",
    moveDown: "Aşağı taşı",
    vs: "vs",
    share: "Paylaş",
    or: "veya",
    offline: "Çevrimdışı",
    retry: "Tekrar Dene",
    errorBoundaryMsg: "Bir şeyler ters gitti. Lütfen tekrar deneyin.",
  },
  tournaments: {
    title: "Turnuvalar",
    newBtn: "Yeni Turnuva",
    needTeamsTitle: "Önce en az 2 takım ekleyin",
    needTeamsNotice: "Önce Takımlar sekmesinde en az 2 takım ekleyin.",
    searchPlaceholder: "Turnuva ara…",
    viewList: "Liste görünümü",
    viewGrid: "Izgara görünümü",
    empty: "Henüz turnuva yok. Başlamak için {action} tıklayın.",
    noMatch: '"{query}" ile eşleşen turnuva yok.',
    live: "Devam Ediyor",
    notFound: "Turnuva bulunamadı.",
    format: {
      groupsKo: "Gruplar+KO",
      groupsKoLong: "Gruplar + KO",
      swiss: "İsviçre",
      custom: "Özel",
      league: "Lig",
      bracket: "Eleme",
    },
    deleteConfirm: "Bu turnuva silinsin mi?",
    deleteLabel: "Sil",
  },
  teams: {
    deleteInUse: "Bu takım bir turnuvada kullanıldığı için silinemez.",
    title: "Takımlar",
    notFound: "Takım bulunamadı.",
    addBtn: "Takım Ekle",
    searchPlaceholder: "Takım ara…",
    empty: "Henüz takım yok. Başlamak için {action} tıklayın.",
    noMatch: '"{query}" ile eşleşen takım yok.',
    limitReached: "Takım limiti doldu ({max})",
    sortDefault: "Varsayılan",
    sortAsc: "Artan",
    sortDesc: "Azalan",
    form: {
      addTitle: "Takım Ekle",
      editTitle: "Takım Düzenle",
      name: "Ad",
      abbreviation: "Kısaltma",
      power: "Güç",
      color: "Renk",
      flag: "Bayrak",
      flagNone: "Bayrak seç…",
      flagPickerTitle: "Bayrak seç",
      flagSearch: "Ülke ara…",
      flagRemove: "Bayrağı kaldır",
      imageSourceTitle: "Görsel seç",
      imageSourceFlag: "Bayrak seç",
      imageSourceUrl: "URL ekle",
      imageSourceGallery: "Galeriden seç",
      imageUrlTitle: "URL ekle",
      imageUrlPlaceholder: "https://…",
      imageRemove: "Görseli kaldır",
      imageInvalid: "Görsel yüklenemedi",
      namePlaceholder: "Takım adı",
      randomName: "Rastgele isim",
    },
    detail: {
      tabs: {
        overview: "Genel Bakış",
        matches: "Maçlar",
      },
      powerRating: "Güç derecesi",
      allStats: "Tüm İstatistikler",
      played: "Oynanan",
      wins: "Galibiyet",
      draws: "Beraberlik",
      losses: "Mağlubiyet",
      winRate: "Galibiyet Oranı",
      goalsFor: "Atılan Gol",
      goalsAgainst: "Yenen Gol",
      titles: "Şampiyonluk",
      recentForm: "Son Form",
      last5: "son 5",
      seasonHistory: "Sezon Geçmişi",
      tournamentTitles: "Turnuva Şampiyonlukları",
      matchHistory: "Maç Geçmişi",
      matchCount: "{n} maç",
      allTournaments: "Tüm turnuvalar",
      seasonN: "Sezon {n}",
      noMatchesPlayed: "Henüz maç oynanmadı.",
      noMatchesTournament: "Bu turnuvada maç yok.",
    },
  },
  players: {
    title: "Oyuncular",
    squadTitle: "Kadro",
    emptyOnTeam: "Bu kadroda henüz oyuncu yok.",
    addBtn: "Ekle",
    searchPlaceholder: "Oyuncu ara…",
    empty: "Henüz oyuncu yok. Başlamak için {action} tıklayın.",
    emptyNoTeams: "Önce bir takım oluşturun, sonra oyuncularını ekleyin.",
    needTeamFirst: "Önce bir takım oluşturun",
    noMatch: "Filtrelere uyan oyuncu yok.",
    deleteAllConfirm: "Tüm oyuncular silinsin mi?",
    sortDefault: "Varsayılan",
    positions: {
      GK: "Kaleci",
      DEF: "Defans",
      MID: "Orta Saha",
      FWD: "Forvet",
    },
    filter: {
      teamLabel: "Takım",
      sortLabel: "Sırala",
      allTeams: "Tüm takımlar",
      searchPlaceholder: "Takım ara…",
      noResults: "Takım bulunamadı",
    },
    form: {
      number: "Numara",
      numberPlaceholder: "—",
      numberPickerTitle: "Forma Numarası Seç",
      numberClear: "Numara Yok",
      addTitle: "Oyuncu Ekle",
      editTitle: "Oyuncuyu Düzenle",
      name: "Ad",
      namePlaceholder: "Oyuncu adı",
      randomName: "Rastgele isim",
      team: "Takım",
      teamPlaceholder: "Takım seçin…",
      position: "Mevki",
      power: "Güç",
    },
    generate: {
      buttonLabel: "Oyuncu Oluştur",
      title: "Oyuncu Oluştur",
      hint: "Kadroda eksik olan mevkileri, 18 oyuncuya kadar doldurur.",
      team: "Takım",
      willAdd: "{count} oyuncu eklenecek:",
      willAddMulti: "{teams} takıma toplam {count} oyuncu eklenecek:",
      squadFull: "Bu kadro zaten tam kapasitede.",
      noTeamsBelow: "Bu kadro büyüklüğünün altında takım yok.",
      fillAllTeams: "Eksik kadrolu tüm takımlara ekle",
      minSquadSize: "Şu değerin altındaki kadrolar",
      minSquadSizeHint: "Oyuncu sayısı bu değerin altındaki her takım doldurulur.",
      firstNames: "İsimler",
      lastNames: "Soyisimler",
      namesPlaceholder: "Her satıra bir isim",
      loadFile: "Dosyadan yükle",
      removeFile: "Dosyayı kaldır",
      fileLoaded: "{file} — {count} isim yüklendi",
      clear: "Sil",
      useDefault: "Varsayılanı kullan",
      generateBtn: "Oluştur",
    },
  },
  history: {
    players: {
      emptyTitle: "Henüz oyuncu kaydı yok",
      emptyDesc: "Bu güncellemeden önce oynanan sezonlarda oyuncu verisi yok.",
      seasonsShort: "Szn",
    },
    title: "Geçmiş",
    empty: "Henüz turnuva yok.",
    noCompletedSeasons: "Henüz tamamlanan sezon yok.",
    tabs: {
      players: "Oyuncular",
      champions: "Şampiyonlar",
      allSeasons: "Tüm Sezonlar",
      allFinals: "Tüm Finaller",
      allTimeTable: "Genel Tablo",
      statistics: "İstatistikler",
      teams: "Takımlar",
    },
    table: {
      season: "Sezon",
      champion: "Şampiyon",
      result: "Sonuç",
      runnerUp: "İkinci",
      thirdPlace: "3. Yer",
      team: "Takım",
      titles: "Şampiyonluk",
      finals: "Finaller",
      seasonsPlayed: "Oynanan Sezonlar",
      seasons: "Sezonlar",
      matchesPlayed: "Oynanan Maçlar",
      played: "Oynanan",
      won: "Galibiyet",
      drawn: "Beraberlik",
      lost: "Mağlubiyet",
      goalsFor: "Atılan Gol",
      goalsAgainst: "Yenen Gol",
      goalDiff: "Gol Farkı",
      points: "Puan",
      cleanSheets: "Gol Yemeden",
    },
    stats: {
      completedSeasons: "Tamamlanan Sezon",
      totalMatches: "Toplam Maç",
      totalGoals: "Toplam Gol",
      goalsPerMatch: "Maç Başına Gol",
      records: "Rekorlar",
      biggestWin: "En Büyük Galibiyet",
      topScorer: "En İyi Golcü",
      goals: "{n} gol",
      cleanSheets: "Gol Yemeden",
      achievements: "Başarılar",
      firstChampion: "İlk Şampiyon",
      season: "Sezon {n}",
      longestStreak: "En Uzun Seri",
      inARow: "{n} üst üste",
      defending: "Şampiyonluğunu Koruyor",
      consecutive: "{n} ardışık",
    },
  },
  settings: {
    title: "Ayarlar",
    language: {
      label: "Dil",
      desc: "Uygulamanın görüntüleme dili",
    },
    appearance: {
      title: "Görünüm",
      design: {
        label: "Tasarım Dili",
        desc: "Yumuşak sakin ve camsıdır; Canlı daha iddialıdır, renk tonlu yüzeyler ve daha yuvarlak köşeler kullanır. Ana renginiz etkilenmez.",
        soft: "Yumuşak",
        vivid: "Canlı",
      },
      theme: {
        label: "Tema",
        desc: "Uygulamanın renk şeması",
        light: "Açık",
        dark: "Koyu",
      },
      primaryColor: {
        label: "Ana Renk",
        desc: "Uygulamanın vurgu rengi",
      },
    },
    tableRules: {
      title: "Varsayılan Tablo Kuralları",
      tiebreaker: {
        label: "Eşitlik Bozucu",
        desc: "Puan eşitliğinde takımları ayıran kural — grup aşaması ve lig tablolarına uygulanır. {h2h} — önce karşılıklı sonuçlar · {goalDiff} — önce genel averaj",
        h2h: "Karşılıklı Maç",
        goalDiff: "Gol Farkı",
        h2hShort: "KM",
        goalDiffShort: "Gol farkı",
      },
    },
    matchDefaults: {
      title: "Maç Varsayılanları",
      intro:
        "Yeni turnuva oluştururken uygulanan varsayılan ayak ayarları. Turnuva başına her zaman değiştirebilirsiniz.",
      groupStage: {
        label: "Grup Aşaması",
        desc: "{single} — çift başına bir maç · {double} — ev sahibi & deplasman, toplam skor",
      },
      knockoutRounds: {
        label: "Eleme Turları",
        desc: "{single} — kazanan ilerler · {double} — 2 maçlık eleme, toplam skor belirler",
      },
      final: {
        label: "Final",
        desc: "Diğer eleme turlarından ayrı ayarlanabilir",
      },
    },
    newTournament: {
      title: "Yeni Turnuva Varsayılanları",
      intro:
        "Turnuva Oluştur penceresinde önceden doldurulmuş değerler. İstediğiniz zaman oradan değiştirebilirsiniz.",
      knockoutDraw: {
        label: "Eleme Kurası",
        desc: "Takımların yalnızca eleme formatındaki brackete yerleştirilme şekli",
      },
      groupDraw: {
        label: "Grup Aşaması Kurası",
        desc: "Takımların gruplara dağıtılma şekli",
      },
      playoffSeeding: {
        label: "Playoff Yerleşimi",
        desc: "Eleme bracketi çekilişinde grup elemelerinin eşleşme şekli",
      },
      drawLegend: {
        draw: "{random} — şans eseri · {seeded} — en iyi takımlar ayrı · {manual} — siz yerleştirin",
        playoff:
          "{cross} — A1 - B2, B1 - A2 · {noRematch} — ilk turda aynı gruptaki rakip yok · {random} — tamamen rastgele",
        cross: "Çapraz",
        noRematch: "Tekrar yok",
        drawOptions: "Kura seçenekleri:",
        playoffSeeding: "Playoff yerleşimi:",
      },
      scoring: {
        title: "Varsayılan Puanlama",
        desc: "Grup/lig turnuvası oluştururken başlangıç değerleri olarak kullanılır",
        winPoints: "Galibiyet",
        drawPoints: "Beraberlik",
        lossPoints: "Mağlubiyet",
      },
    },
    graphics: {
      title: "Grafik",
      bracketQuality: {
        label: "Bracket Kalitesi",
        desc: "Yüksek — tüm yakınlaştırma seviyelerinde keskin görüntü · Düşük — GPU hızlandırmasını etkinleştirir, yakınlaştırmada netliği azaltabilir",
        high: "Yüksek",
        low: "Düşük",
      },
    },
    effects: {
      title: "Kazanma Efektleri",
    },
    display: {
      title: "Görüntü",
      teamAbbr: {
        label: "Takım Kısaltmaları",
        desc: "{example} gibi kısa kod — bracket slotlarında ve dar düzenlerde gösterilir",
      },
      bracketStyle: {
        label: "Bracket Stili",
        desc: "{doubleSided} — takımlar brackette her iki tarafta · {classic} — soldan sağa tek bracket · {auto} — 17+ eleme takımı için Çift Taraflı, aksi halde Klasik",
        doubleSided: "Çift Taraflı",
        classic: "Klasik",
        auto: "Otomatik",
      },
      confetti: {
        label: "Şampiyon Konfetisi",
        desc: "Turnuva bittiğinde kazananın rengiyle konfeti fırlat",
      },
      sound: {
        label: "Kazanan Sesi",
        desc: "Turnuva bittiğinde bir ses çal",
      },
      bracketHover: {
        label: "Bracket Yol Vurgusu",
        desc: "Brackette bir takımın üzerine gelince yolu aydınlansın, diğerleri soluklaşsın",
      },
      bracketConnectorColors: {
        label: "Bracket Bağlantı Renkleri",
        desc: "Bracket bağlantı çizgilerini ilerleyen takımların renkleriyle boyar",
      },
      gradualReveal: {
        label: "Kademeli Sonuç Açılımı",
        desc: "Maç sonuçları hepsi birden değil, kısa gecikmelerle teker teker gösterilsin",
      },
      autoAdvanceFixtureStage: {
        label: "Fikstürde Otomatik İlerleme",
        desc: "Fikstür sekmesinde bir aşamayı simüle ettikten sonra otomatik olarak sıradakine geç",
      },
      liveEvents: {
        label: "Canlı Maç Olayları",
        desc: "Canlı maç akışında hangi olay türlerinin görüneceğini seç",
      },
    },
    simulation: {
      redCardImpact: {
        label: "Kırmızı Kart Etkisi",
        desc: "Kırmızı kart gören takım maçın kalanında zayıflar ve sonraki maçta küçük bir dezavantaj taşır.",
      },
      injuries: {
        label: "Sakatlıklar",
        desc: "Bir sakatlık, oyuncuyu takımının sonraki birkaç maçında rastgele bir süre kadro dışı bırakabilir.",
      },
      title: "Simülasyon",
      homeAdvantage: {
        label: "Ev Sahibi Avantajı",
        desc: "Ev sahibi takıma verilen güç bonusu. {zero} = avantaj yok · {default} = varsayılan · {max} = çok güçlü ev avantajı.",
        neutral: "Tarafsız",
        slight: "Hafif",
        moderate: "Orta",
        strong: "Güçlü",
        dominant: "Baskın",
      },
      surpriseFactor: {
        label: "Sürpriz Faktörü",
        desc: "Zayıf takımların güçlü takımları yenebileceği oran. {zero} = güç tamamen belirler · {max} = tamamen şans.",
        predictable: "Öngörülebilir",
        pureLuck: "Saf şans",
        mostlySkill: "Ağırlıklı beceri",
        upsetHeavy: "Sürpriz ağırlıklı",
        balanced: "Dengeli",
      },
      formFactor: {
        label: "Form Faktörü",
        desc: "Son 5 maç sonucuna göre takım gücünü dinamik olarak ayarlar. Galibiyet serisi yapan takım {plus} güç kazanır, sürekli kaybeden takım {minus} düşer.",
      },
      usePlayerPower: {
        label: "Oyuncu Gücünü Kullan",
        desc: "Takımın gücünü, kadronun ortalama oyuncu gücüyle harmanlar. Kapalıysa sadece takımın kendi gücü kullanılır.",
      },
    },
    sampleData: {
      withSquads: "Kadrolu",
      title: "Örnek Veriler",
      countries: "Ülkeler",
      clubs: "Kulüpler",
      intro:
        "Hazır takım listesi yükleyin. Bu işlem mevcut takımlarınızın yerini alır ve tüm turnuvaları siler.",
      loadConfirm:
        '"{name}" veri seti yüklensin mi? Bu işlem takımlarınızı değiştirecek ve tüm turnuvaları silecek.',
      loadLabel: "Yükle",
    },
    dataManagement: {
      title: "Veri Yönetimi",
      backup: {
        label: "Yedekleme",
        desc: "Tüm takım ve turnuvalarınızı bir dosyaya kaydedin veya bir dosyadan geri yükleyin",
      },
      clearAll: {
        label: "Tüm Verileri Temizle",
        desc: "Tüm takımları ve turnuvaları kalıcı olarak sil — geri alınamaz",
        confirmMsg: "Tüm verileri silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.",
        confirmLabel: "Tümünü Temizle",
        btn: "Tümünü Temizle",
      },
      invalidFile: "Geçersiz yedek dosyası.",
      partialImport: "İçe aktarıldı, ancak {count} turnuva okunamadı ve atlandı.",
    },
    resetSettings: {
      title: "Ayarları Sıfırla",
      label: "Varsayılana Sıfırla",
      desc: "Tüm ayarları orijinal değerlerine geri yükleyin. Takımlar ve turnuvalar etkilenmez.",
      btn: "Sıfırla",
    },
    nav: {
      appearance: "Dil ve Görünüm",
      bracket: "Bracket ve Efektler",
      tournament: "Turnuva Kuralları",
      simulation: "Simülasyon",
      music: "Müzik",
      data: "Veriler",
    },
    tutorials: "Eğitim Videoları",
    about: "Hakkında",
    aboutBy: "geliştirici",
    specialThanks: "Özel teşekkürler",
  },
  tournament: {
    exportExcel: "Excel'e aktar",
    noFixtures: "Henüz maç yok.",
    groupStageNotStarted: "Grup aşaması henüz başlamadı.",
    matchdayProgress: "{played}/{total} hafta",
    champion: "Şampiyon",
    simulateMatchday: "Haftayı simüle et",
    wildcardRace: "Joker Yarışı",
    aggregate: "Toplam",
    notFound: "Turnuva bulunamadı.",
    newSeason: "Yeni Sezon",
    simulateAll: "Tümünü Simüle Et",
    simulate: "Simüle Et",
    simulateRound: "Simüle Et: {round}",
    simulateThirdPlace: "3.'lük Maçını Simüle Et",
    simulateWeek: "Haftayı Simüle Et",
    simulateGroup: "{group} Simüle Et",
    advanceToKnockout: "Eleme Aşamasına Geç",
    settings: "Ayarlar",
    previousSeason: "Önceki sezon",
    nextSeason: "Sonraki sezon",
    fixtures: {
      groupWeek: "{n}. Hafta",
      simulateStage: "Bu aşamayı simüle et",
    },
    locked: {
      bracketNeedsGroups: "Elemelerin açılması için grup aşamasını tamamlayın.",
      bracketNeedsPlayoff: "Play-off'un açılması için sezonu tamamlayın.",
      stats: "İstatistikleri görmek için birkaç maç oynayın.",
    },
    tabs: {
      groups: "Gruplar",
      bracket: "Eleme",
      league: "Lig",
      stats: "İstatistikler",
      participants: "Katılımcılar",
      wildcards: "Wildcards",
      standings: "Puan Durumu",
      fixtures: "Fikstür",
      playoff: "Play-off",
      swissPhase: "Lig Aşaması",
    },
    header: {
      teams: "{n} takım",
      created: "{date} tarihinde oluşturuldu",
    },
    newSeasonModal: {
      useOldDraw: "Eski Kurayı Kullan",
      useOldDrawDesc: "Sezon {n} kura sıralamasını koru",
      newDraw: "Yeni Kura",
      newDrawDesc: "Takım ekle veya çıkar, sonra kura çek",
      currentTeams: "Turnuvadaki Takımlar",
      availableTeams: "Müsait Takımlar",
      noAvailable: "Müsait takım yok",
      noTeams: "Takım seçilmedi",
    },
    phases: {
      title: "Aşamalar",
      help: "Bir aşamanın sağ portunu sonraki aşamanın sol portuna sürükle.",
      summary: "{count} aşama",
      summaryEmpty: "Henüz aşama yok",
      button: "Aşamalar",
      kinds: {
        group: "Gruplar",
        league: "Lig",
        swiss: "İsviçre",
        knockout: "Eleme",
      },
      node: {
        name: "Aşama adı",
        teams: "{count} takım",
        config: "Ayarlar",
        final: "Final",
        remove: "Aşamayı kaldır",
      },
      guide: {
        lead: "Turnuvayı aşamalardan kur ve birbirine bağla.",
        add: "Yukarıdaki butonlardan aşama ekle — grup, lig, İsviçre turu veya eleme.",
        connect:
          "Bir aşamanın sağ portundan sonraki aşamanın sol portuna sürükle. Dokunmatik ekranda önce bir porta, sonra diğerine dokun.",
        configure:
          "Aşamanın Ayarlar’ından yapısını, bağlantısından hangi sıraların yükseleceğini belirle.",
        final:
          "Kazananı belirleyen aşamayı Final olarak işaretle. Elemenin çıkış portu yoktur — dal orada biter.",
        example:
          "Sık kullanılan şema: Gruplar → Lig → Kupa. Gruplar yükselenleri lige gönderir, ligin ilk dördü kupaya gider, kupa finaldir.",
        dismiss: "Anladım",
        reopen: "Bu nasıl çalışıyor?",
      },
      links: {
        title: "Bağlantılar",
        edit: "Hangi sıraların yükseleceğini değiştir",
        remove: "Bağlantıyı kaldır",
      },
      edge: {
        single: "{rank}. sıra",
        range: "{from}-{to}. sıralar",
        teams: "{count} takım",
      },
      range: {
        title: "Kimler yükselir",
        subtitle: "{from} → {to}",
        from: "Başlangıç sırası",
        to: "Bitiş sırası",
        hint: "{count} takım {to} aşamasına geçer.",
        disconnect: "Bağlantıyı kaldır",
        groupNote:
          "{from} kimlerin yükseleceğini kendi ayarlarında belirler — grup başına yükselen sayısı ve wildcard.",
      },
      config: {
        intake: "Bu aşamaya {count} takım geliyor",
        shape: "Yapı",
        groupCount: "Grup sayısı",
        seedMode: "Yerleştirme",
        legs: "Eşleşme başına maç",
        opponents: "Takım başına rakip",
        pots: "Torba sayısı",
        balance: "İç saha ve deplasmanı dengele",
        finalLegs: "Final",
        thirdPlace: "Üçüncülük maçı",
        thirdPlaceLegs: "Üçüncülük maç sayısı",
        swissSize: "{matchdays} hafta",
        qualification: "Yükselme",
        qualifiersPerGroup: "Grup başına yükselen",
        wildcards: "Wildcard",
        wildcardsHint: "Bir sıra altta kalanların en iyileri",
        advancing: "{count} takım yükselir",
      },
      errors: {
        noPhases: "En az bir aşama ekleyin.",
        noEntry: "Bir aşamanın girişi boş olmalı — takımlar oradan girer.",
        multipleEntry: "Yalnızca bir aşama başlangıç aşaması olabilir.",
        cycle: "Bağlantılar bir döngü oluşturuyor.",
        unnamed: "Bu aşamaya bir isim verin.",
        knockoutHasOutput: "Eleme aşaması takım ilerletemez.",
        noFinal: "Bir aşamayı final olarak işaretleyin.",
        multipleFinal: "Yalnızca bir aşama final olabilir.",
        finalNotTerminal: "Final aşaması takım ilerletemez.",
        edgeRankRange: "{from}-{to}. sıralar yok — bu aşamada {available} takım var.",
        rangeOverlap: "İki bağlantı aynı sıraları istiyor.",
        phaseTooSmall: "{intake} takım yetersiz — bu aşama en az {min} takım gerektirir.",
        groupCount: "En az iki grup belirleyin.",
        groupsTooSmall: "{intake} takım {groupCount} grubu doldurmaz.",
        groupSingleOutput:
          "Grup aşaması yükselenleri tek küme olarak gönderir — yalnızca bir bağlantısı olabilir.",
        groupQualifiers: "Grup başına {qualifiers} fazla — en çok {max}.",
        groupWildcards: "{wildcards} wildcard fazla — en çok {max}, grup başına bir tane.",
        swissOpponents: "{opponents} rakip fazla — en çok {max}.",
        configMismatch: "Bu aşamanın ayarları türüyle uyuşmuyor.",
        danglingEdge: "Bir bağlantı silinmiş bir aşamayı gösteriyor.",
        selfEdge: "Bir aşama kendisine bağlanamaz.",
      },
      advance: "{phase} aşamasına geç",
      locked: "Bu aşamanın açılması için {phase} bitmeli.",
      notStarted: "Bu aşama henüz başlamadı.",
    },
    create: {
      title: "Yeni Turnuva",
      name: "Turnuva Adı",
      namePlaceholder: "örn. Bahar Şampiyonası 2025",
      randomName: "Rastgele isim",
      teams: "Katılımcı Takımlar",
      selected: "({n} seçildi)",
      selectAll: "Tümünü Seç",
      deselectAll: "Seçimi Kaldır",
      minTeams: "En az 2 takım seçin.",
      format: "Turnuva Formatı",
      formats: {
        bracket: "Eleme Bracketi",
        bracketDesc: "Bir kez kaybet ve elenirsin",
        groupsKo: "Gruplar + Eleme",
        groupsKoDesc: "Grup aşaması, ardından en iyi takımlar ilerler",
        league: "Lig",
        leagueDesc: "Herkes herkesle oynar, en fazla puan kazanır",
        leagueKo: "Lig + Eleme",
        leagueKoDesc: "Normal sezon, ardından en iyi takımlar play-off oynar",
        swiss: "İsviçre Sistemi",
        swissDesc:
          "Tek puan tablosu, her takım sahanın yalnızca bir kısmıyla oynar (Şampiyonlar Ligi tarzı)",
        custom: "Özel",
        customDesc: "Kendi aşamalarını kur — grup, lig ve eleme, istediğin sırayla",
      },
      swissConfig: {
        phase: "Lig Aşaması",
        opponents: "Takım başına rakip",
        opponentsHint: "Her takımın kaç farklı rakiple oynayacağı",
        legs: "Rakip başına maç",
        homeAwayBalance: "Ev-deplasman dengesi",
        draw: "Kura",
        drawType: "Yöntem",
        drawSeeded: "Güce göre (pot)",
        drawRandom: "Rastgele",
        pots: "Pot sayısı",
        potsHint: "Pot başına {size} takım · her pottan {perPot} rakip",
        potsNone: "Tek pot — seri başı yok",
        summary: "{matchdays} maç günü · {matches} maç",
        lockedBanner: "Sonuç girildikten sonra kilitlenir",
        errors: {
          minTeams: "En az 4 takım gerekli",
          minOpponents: "En az 1 rakip gerekli",
          tooManyOpponents: "Bir takım, diğer takım sayısından fazla rakiple oynayamaz",
          oddProduct: "Takım sayısı × rakip sayısı çift olmalı",
          potSizeUneven: "Potlar takımları eşit bölmeli",
          potDivision: "Rakip sayısı pot sayısına tam bölünmeli",
          oddPotQuota: "Pot büyüklüğü × pot başına rakip çift olmalı",
        },
      },
      config: {
        group: "Grup Yapılandırması",
        knockout: "Eleme Yapılandırması",
        league: "Lig Yapılandırması",
        groupsAndAdvance: "{groups} grup · {advance} tur atlıyor",
        wildcardsShort: "+{n} joker",
        thirdPlaceOn: "3.'lük maçı var",
        noThirdPlace: "3.'lük maçı yok",
        legsShort: "Eleme {ko} · Final {final}",
        divisionsShort: "{n} lig",
        singleDivisionShort: "tek lig",
        playoffShort: "{n} takımlık play-off",
        playoffOff: "Play-off aşaması yok",
        swiss: "İsviçre Ayarları",
        swissOpponentsShort: "{n} rakip",
        swissPotsShort: "{n} pot",
      },
      groups: "Grup Sayısı",
      groupSizeEven: "{size} takımlı {count} grup",
      groupSizeSplit: "{sizeA} takımlı {countA} grup · {sizeB} takımlı {countB} grup",
      advance: "Elemeye yükselen takımlar",
      perGroup: "gruptan",
      reachKnockout: "elemeye ulaşır",
      wildcards: "En iyi ikinci wildcards",
      total: "toplam",
      roundFormat: "Tur Formatı",
      schedule: "Program",
      leagueHint:
        "{single} — bir kez · {double} — ev & deplasman · {triple} — 3 maç · {quad} — 4 maç (2E & 2D)",
      tiers: "Lig Kademeleri",
      numberOfTiers: "Kademe Sayısı",
      singleDivision: "tek lig",
      divisions: "{n} lig",
      promotionRelegation: "Yükselme / Küme Düşme",
      teamsSwap: "takımlar komşu kademeler arasında yer değiştirir",
      playoff: {
        enable: "Play-off aşaması ekle",
        hint: "Ligin ardından en iyi takımlar eleme usulü bir play-off oynar.",
        qualifierCount: "Play-off'a çıkan takımlar",
        qualifierCountHint: "final tablosunda play-off'a kalan en iyi takımlar",
        seedMode: "Eşleştirme",
      },
      drawMethod: "Kura Yöntemi",
      groupStage: "Grup aşaması",
      playoffBracket: "Playoff bracketi",
      drawHint:
        "{random} — şans eseri · {seeded} — en iyi takımlar ayrı · {manual} — siz düzenleyin",
      playoffHint:
        "{cross} — A1 - B2, B1 - A2 · {noRematch} — ilk turda aynı gruptaki eşleşme yok · {random} — tamamen rastgele",
      cross: "Çapraz",
      noRematch: "Tekrar yok",
      options: "Seçenekler",
      thirdPlace: "3. lük Maçı",
      thirdPlaceHint: "Yarı final kaybedenler bronz madalya için oynar",
      tiebreaker: "Eşitlik Bozucu",
      createBtn: "Turnuva Oluştur",
    },
    setResult: "Sonuç Gir",
    simulationResults: {
      noResults: "Simülasyon sonucu bulunamadı.",
      runFirst: "Önce bir simülasyon çalıştırın",
      title: "Simülasyon Sonuçları",
      runs: "{n} deneme",
      season: "Sezon {n}",
      format: "Format",
      teams: "Takımlar",
      simulations: "Simülasyonlar",
      championshipOdds: "Şampiyonluk İhtimalleri",
      leagueOdds: "Lig Simülasyon İhtimalleri",
      winProbability: "Kazanma ihtimali",
      titleProbability: "Şampiyonluk ihtimali",
      win: "Kazanma",
      final: "Final",
      top4: "Top 4",
      adv: "İler.",
      titleCol: "Şampiyon",
      top3: "Top 3",
      avgPts: "Ort. Puan",
      avgGF: "Ort. Atılan",
      avgGA: "Ort. Yenen",
      saveJson: "JSON kaydet",
      saveCsv: "CSV kaydet",
    },
    settingsPage: {
      title: "Turnuva Ayarları",
      notFound: "Turnuva bulunamadı.",
      backToTournaments: "Turnuvalara Geri Dön",
      locked: "Kilitli",
      saveChanges: "Değişiklikleri Kaydet",
      tournamentName: {
        title: "Turnuva Adı",
        hint: "Yeniden adlandırma bu ismi paylaşan {n} sezonu da güncelleyecek.",
      },
      info: {
        created: "{date} tarihinde oluşturuldu",
        teams: "{n} takım",
        champion: "Şampiyon",
      },
      manageTeams: {
        title: "Takımları Yönet",
        addPlaceholder: "Takım ekle…",
        add: "Ekle",
        allAdded: "Mevcut tüm takımlar zaten bu turnuvada.",
        lockedBanner: "Maç oynandıktan sonra takım yönetimi devre dışıdır.",
      },
      drawMethod: {
        lockedBanner: "Maçlar başladıktan sonra kura değiştirilemez.",
        regenerate: "↺ Yeniden Çek",
        redrawConfirm: "Kura sıfırlanıp yeniden çekilsin mi?",
        redrawConfirmLabel: "Yeniden Çek",
      },
      groupStructure: {
        title: "Grup Yapısı",
        lockedBanner: "Maçlar başladıktan sonra grup yapısı değiştirilemez.",
      },
      qualification: {
        title: "Kalifikasyon ve Wildcard",
        teamsAdvance: "Gruptan çıkan takım sayısı",
        reachKnockout: "→ {n} nakavt aşamasına geçer",
        total: "→ {n} toplam",
        lockedBanner: "Kilitli — nakavt bracketi gruplardan zaten oluşturuldu.",
      },
      playoffSeeding: {
        title: "Playoff Eşleşmesi",
        lockedBanner: "Kilitli — grup aşaması tamamlandı ve bracket oluşturuldu.",
      },
      formatOptions: {
        title: "Format Seçenekleri",
        lockedBanner: "Maçlar başladıktan sonra format değiştirilemez.",
      },
      legsPerMatch: {
        title: "Maç Başına Oturum",
        hintSingle: "1 maç, kazanan ilerler",
        hintDouble: "ev ve deplasman, toplam skora göre",
        r64: "Son 64",
        r32: "Son 32",
        r16: "Son 16",
        quarterFinal: "Çeyrek Final",
        semiFinal: "Yarı Final",
        final: "Final",
        thirdPlace: "3.'lük Maçı",
        lockedBanner: "Maçlar başladıktan sonra oturum ayarları değiştirilemez.",
      },
      leagueFormat: {
        title: "Lig Formatı",
        hintSingle: "bir kez",
        hintDouble: "ev ve deplasman",
        hintTriple: "3 karşılaşma",
        hintQuad: "4 karşılaşma (2E & 2D)",
        lockedBanner: "Maçlar başladıktan sonra format değiştirilemez.",
        numberOfDivisions: "Bölüm Sayısı",
        minTeams: "bölüm başına min 2 takım",
        slotsSwapped: "komşu bölümler arasında yer değiştirme",
        divisions: {
          title: "Bölümler",
          lockedBanner: "Maçlar başladıktan sonra bölümler değiştirilemez.",
        },
        playoff: {
          title: "Playoff",
          lockedBanner: "Playoff başladıktan sonra playoff ayarları değiştirilemez.",
        },
      },
      tiebreaker: {
        method: "Yöntem",
        h2h: "H2H",
        goalDiff: "Gol farkı",
      },
      scoring: {
        title: "Puanlama",
        winPoints: "Galibiyet Puanı",
        drawPoints: "Beraberlik Puanı",
        lossPoints: "Mağlubiyet Puanı",
        lockedBanner: "Maçlar başladıktan sonra puanlama değiştirilemez.",
      },
      simulation: {
        title: "Monte Carlo Simülasyonu",
        desc: "Bu turnuvayı 10.000 kez simüle et ve her takım için kazanma ihtimallerini gör.",
        run: "10.000× Simüle Et",
        simulating: "{name} simüle ediliyor…",
        done: "Simülasyon tamamlandı — 10.000 çalıştırma bitti.",
        viewResults: "Sonuçları Gör",
      },
      teamAdjustments: {
        title: "Takım Ayarlamaları",
        pointsTitle: "Takım Puan Ayarlamaları",
        powerTitle: "Takım Güç Ayarlamaları",
        pointsHint: "Bu sezon belirli takımlara puan ekle veya çıkar. Yeni sezonda sıfırlanır.",
        powerHint:
          "Bu sezon simülasyon için takımların güç puanını geçici olarak artır veya azalt. Yeni sezonda sıfırlanır.",
        lockedBanner: "Maçlar başladıktan sonra ayarlamalar değiştirilemez.",
      },
      dangerZone: {
        title: "Tehlikeli Bölge",
        resetLabel: "Turnuvayı Sıfırla",
        resetDesc: "Tüm maç sonuçlarını siler, takımlar ve kura korunur.",
        resetConfirm: "Bu turnuva sıfırlansın mı?",
        resetConfirmLabel: "Sıfırla",
        deleteLabel: "Turnuvayı Sil",
        deleteDesc: "Bu turnuvayı ve tüm verilerini kalıcı olarak kaldırır.",
        deleteConfirm: "Bu turnuva silinsin mi?",
        deleteConfirmLabel: "Sil",
      },
      unsavedChanges: {
        title: "Kaydedilmemiş Değişiklikler",
        message: "Kaydedilmemiş değişiklikleriniz var. Ne yapmak istersiniz?",
        leave: "Çık",
        saveAndLeave: "Kaydet ve Çık",
        close: "Kapat",
      },
    },
  },
  teamSelector: {
    searchPlaceholder: "Takım ara…",
    selected: "Seçilen",
    available: "Seçilebilecek",
    selectAll: "Tümünü seç",
    deselectAll: "Tümünü kaldır",
    emptySelected: "Takım seçilmedi",
    emptyAvailable: "Takım bulunamadı",
    minTeams: "En az 2 takım seçin",
    sortName: "İsim",
    sortPower: "Güç",
    title: "Takımlar",
    fullView: "Tam ekran",
  },
  manualDraw: {
    byesTitle: "Bay Geçenler (otomatik tur atlama)",
    bye: "BAY",
    matches: "Eşleşmeler",
    home: "Ev Sahibi",
    away: "Deplasman",
    confirmDraw: "Kura Çekimini Onayla",
    groupDrawHint:
      "Takımları gruplara sürükleyin veya bir takıma, ardından bir alana dokunarak atayın.",
    groupLabel: "Grup {name}",
    poolTitle: "Uygun takımlar",
    progress: "{done}/{total} atandı",
    dropHere: "Takımı buraya bırakın veya dokunun",
    wildcard: "Wildcard",
    remove: "Kaldır",
  },
  leaguePlayoff: {
    startPlayoff: "Play-off'u Başlat",
    finishSeasonFirst: "Play-off'u başlatmak için sezonu tamamlayın.",
    qualifier: "Sıra {rank}",
    playoffDrawTitle: "Play-off Kurası",
    potTop: "Seri başları",
    potRest: "Diğerleri",
  },
  coach: {
    title: "Teknik Direktör",
    appoint: "Ata",
    empty: "Teknik direktör yok. Takım standart oynuyor.",
    styles: {
      attacking: "Hücum",
      balanced: "Dengeli",
      defensive: "Defans",
    },
    styleHints: {
      attacking: "Daha çok gol atar, daha çok gol yer.",
      balanced: "Ekstra bir talimat yok.",
      defensive: "Daha az gol yer, daha az gol atar.",
    },
    form: {
      title: "Teknik Direktör",
      hint: "Formasyon, oyun stili ve teknik direktörün gücü bu takımın maçlarını doğrudan etkiler.",
      name: "İsim",
      namePlaceholder: "Teknik direktör adı",
      formation: "Formasyon",
      formationHint: "Takımın sahaya çıktığı dizilim.",
      style: "Oyun stili",
      power: "Teknik direktör gücü",
      randomize: "Rastgele",
      sack: "Kaldır",
    },
    generate: {
      title: "Teknik Direktör Üret",
      buttonLabel: "Teknik direktör üret",
      hint: "Her takıma, kulübün gücüne yakın güçte bir teknik direktör atar.",
      overwrite: "Mevcut teknik direktörleri değiştir",
      willAdd: "{count} teknik direktör atanacak",
      keeping: "{count} korunuyor",
      allCoached: "Her takımın zaten teknik direktörü var",
      generateBtn: "Üret",
    },
  },
  manager: {
    match: {
      title: "Menajer",
      manage: "Yönet",
    },
    tactics: {
      title: "Taktik",
      hint: "Bir sonraki dakikadan itibaren geçerli. Oynanan dakikalar değişmez.",
      apply: "Uygula",
    },
    sub: {
      title: "Oyuncu Değişikliği",
      pickOut: "Kim çıkıyor?",
      pickIn: "Kim giriyor?",
      back: "Geri",
      left: "{count} hak",
      noneLeft: "Değişiklik hakkı kalmadı",
      emptyBench: "Yedek kulübesi boş",
      unknownPlayer: "Bilinmeyen Oyuncu",
    },
    banner: {
      title: "{team} — sıradaki maçın",
      play: "Yönet",
    },
    settings: {
      title: "Menajer Modu",
      team: "Bir takımı yönet",
      hint: "Bu takımın tüm sezon boyunca her maçını sen oynarsın. Toplu simülasyon seni bekler.",
      placeholder: "Kimseyi yönetme — hepsi simüle edilsin",
      standDown: "Görevi bırak",
      blocked: "Önce kendi maçını yönet",
    },
    panel: {
      allPlayed: "Oynanacak maçın kalmadı — geri kalan her şey otomatik oynanır.",
    },
    lineup: {
      title: "İlk 11",
      count: "{n}/11",
      hint: "İlk onbirini mevkilerine göre seç — tüm formalar dolmadan maç başlamaz.",
      noSquad: "Bu takımda kayıtlı oyuncu yok.",
      noneForPosition: "Bu mevkide oyuncu yok.",
      incomplete: "Maça çıkmadan önce {n} oyuncu daha seçmelisin.",
      injured: "Sakat",
      suspended: "Cezalı",
    },
  },
  music: {
    title: "Arka Plan Müziği",
    builtIn: "Gömülü",
    tracksTitle: "Parçalar",
    tracksHint:
      "Kendi ses dosyalarını ekle, her biri en fazla {max} MB. Sadece bu cihazda saklanır.",
    addTrack: "Parça ekle",
    manage: "Yönet",
    manageDesc: "Kendi parçalarını ekle veya kaldır",
    invalidType: "Bu bir ses dosyası değil.",
    tooLarge: "Bu dosya {max} MB sınırını aşıyor.",
    saveFailed: "Parça kaydedilemedi.",
    enabled: {
      label: "Müzik çal",
      desc: "Oynarken arkada sakin bir döngü. Varsayılan olarak açık.",
    },
    volume: {
      label: "Ses seviyesi",
      desc: "Müziğin uygulamanın altında ne kadar duyulacağı",
      silent: "Sessiz",
      quiet: "Kısık",
      gentle: "Yumuşak",
      loud: "Yüksek",
    },
    track: {
      label: "Parça",
      desc: "Uygulama açıkken çalan parça",
    },
    loop: {
      label: "Tek parçayı tekrarla",
      desc: "Kapalıyken listedeki tüm parçalar sırayla çalar",
    },
  },
  liveMatch: {
    title: "Canlı",
    replayTitle: "Tekrar",
    watch: "Canlı",
    replay: "Tekrar izle",
    skip: "Atla",
    pause: "Duraklat",
    resume: "Devam et",
    kickOff: "Başlama vuruşu",
    halfTime: "Devre arası",
    fullTime: "Maç sonu",
    extraTime: "Uzatmalar",
    extraTimeShort: "UZT",
    penalties: "Penaltılar",
    speedOption: "{value}x",
    useResult: "Sonucu al",
    settingsLabel: "Canlı Maç Hızı",
    settingsDesc: "Maçı canlı izlerken ne kadar hızlı oynandığı",
  },
  matchStats: {
    aetFull: "Uzatmalar sonunda · 90'da {home}–{away}",
    aet: "uzt.",
    shootoutShort: "Penaltılar",
    shootoutTitle: "Penaltı Atışları",
    manOfTheMatch: "Maçın adamı",
    title: "Maç Raporu",
    buttonLabel: "Rapor",
    comparison: "Maç İstatistikleri",
    timeline: "Maç Akışı",
    lineups: "Oyuncu Puanları",
    possession: "Topla Oynama",
    shots: "Şut",
    onTarget: "İsabetli Şut",
    xg: "xG",
    bigChances: "Net Gol Fırsatı",
    corners: "Korner",
    offsides: "Ofsayt",
    fouls: "Faul",
    shootout: "Penaltılar {home}–{away}",
    unknownPlayer: "Bilinmeyen Oyuncu",
    assistBy: "Asist: {name}",
    noEvents: "Bu maçta olay yok.",
    legacyNotice: "Bu maç, maç raporları eklenmeden önce oynandı.",
    player: "Oyuncu",
    goalsShort: "G",
    goalsFull: "Gol",
    assistsShort: "A",
    assistsFull: "Asist",
    ratingShort: "Puan",
    home: "Ev Sahibi",
    away: "Deplasman",
    events: {
      goal: "Gol",
      penGoal: "Penaltı",
      ownGoal: "Kendi kalesine",
      penMiss: "Penaltı kaçtı",
      yellow: "Sarı kart",
      red: "Kırmızı kart",
      sub: "Oyuncu değişikliği",
      subFor: "{name} yerine oyuna girdi",
      penShort: "(P)",
      ownShort: "(KK)",
    },
  },
  playerStats: {
    scorers: "Gol Krallığı",
    assists: "Asist",
    rated: "Puanlar",
    keepers: "Kaleciler",
    player: "Oyuncu",
    colGoals: "G",
    colAssists: "A",
    colRating: "Puan",
    colCleanSheets: "GY",
    colConceded: "YG",
    colSaves: "Krt",
    colApps: "OM",
    emptyTitle: "Henüz oyuncu istatistiği yok",
    emptyDesc: "Gol atanları görmek için bir maç oyna.",
    emptyHint: "Burada sıralanacak bir şey yok.",
    emptyRated: "Puanlar {count} maçtan sonra görünür.",
  },
  playerDetail: {
    yellowCards: "Sarı",
    redCards: "Kırmızı",
    injuries: "Sakatlık",
    matchesMissedToInjury: "Sakatlıktan kaçırılan",
    notFound: "Oyuncu bulunamadı.",
    careerTitle: "Kariyer",
    honoursTitle: "Başarılar",
    apps: "Maç",
    goals: "Gol",
    assists: "Asist",
    saves: "Kurtarış",
    conceded: "Yenilen",
    cleanSheets: "Gol yemedi",
    avgRating: "Ortalama puan",
    bestRating: "En yüksek puan",
    honourTitle: "Şampiyon",
    honourTopScorer: "Gol kralı — {goals} gol",
    emptyTitle: "Henüz maç yok",
    emptyDesc: "Bu oyuncu sahaya çıkınca istatistikler görünür.",
  },
  stats: {
    teamStats: "Takım İstatistikleri",
    playerStats: "Oyuncu İstatistikleri",
    round: "{round}. Tur",
    league: "Lig",
    group: "Grup",
    standingsProgress: "{name} — Puan Durumu",
    defaultStandingsProgress: "Puan Durumu",
    topScorers: "Gol Krallığı",
    bestDefense: "En İyi Savunma",
    team: "Takım",
    goalsForTitle: "Atılan Gol",
    gf: "AG",
    goalsAgainstTitle: "Yenilen Gol",
    ga: "YG",
    matchesPlayedTitle: "Oynanan Maç",
    mp: "OM",
  },
  progressChart: {
    title: "Puan / Sıra Gelişimi",
    position: "Sıra",
    points: "Puan",
    pts: "puan",
  },
  legend: {
    qualifies: "Tur Atladı",
    wildcard: "Wildcard (en iyi {count})",
    eliminated: "Elendi",
  },
  participants: {
    rank: "#",
    team: "Takım",
    group: "Grup",
    wins: "G",
    draws: "B",
    losses: "M",
    gf: "AG",
    ga: "YG",
    gd: "AV",
    power: "GÜÇ",
    result: "Sonuç",
    place: "{ordinal} Sıra",
    eliminated: "Elendi · {round}",
  },
  rounds: {
    matchday: "{n}. Hafta",
    final: "Final",
    thirdPlace: "3.lük Maçı",
    semiFinals: "Yarı Final",
    quarterFinals: "Çeyrek Final",
    roundOf: "Son {n}",
  },
  notFound: {
    message: "Sayfa bulunamadı",
    goHome: "Ana sayfaya dön",
  },
} as const
