export default {
  drawCeremony: {
    potLabel: "Bombo {n}",
    potSingle: "Bombo",
    byeSlot: "BYE {n}",
    matchSlot: "Partido {n}",
    wildcardsPot: "Comodines",
    groupWinnersPot: "Primeros de grupo",
    runnersUpPot: "Segundos de grupo",
    rankPot: "Puesto {n}",
    title: "Sorteo",
    potsHint: "Arrastra los equipos entre bombos y luego inicia el sorteo.",
    crossLocked: "Sorteo cruzado: los enfrentamientos son fijos. Inicia el sorteo para revelarlos.",
    resetPots: "Restablecer bombos",
    startDraw: "Iniciar Sorteo",
    useOldDraw: "Usar sorteo anterior",
    manageTeams: "Equipos",
    skipDraw: "Saltar Sorteo",
    pauseDraw: "Pausar",
    resumeDraw: "Reanudar",
    continue: "Continuar",
    speed: "Velocidad",
    speedNormal: "Normal",
    speedFast: "Rápida",
    settingsLabel: "Sorteo",
    settingsDesc: "Mostrar un sorteo animado al asignar equipos a grupos o al cuadro",
    errors: {
      unassigned: "Todos los equipos deben estar asignados a un bombo.",
      duplicate: "Un equipo aparece en más de un bombo.",
      emptyPot: "Los bombos no pueden estar vacíos.",
    },
    playoffDrawTitle: "Sorteo de playoff",
  },
  matchScore: {
    penalties: "Tanda de penaltis",
    penaltiesHint: "La eliminatoria está empatada — los penaltis deciden quién pasa.",
    penaltiesTied: "Una tanda de penaltis no puede acabar en empate.",
    simulate: "Simular",
    clear: "Borrar resultado",
  },
  bracket: {
    fitScreen: "Ajustar a pantalla",
    zoomHint: "Pellizca para hacer zoom · doble toque para restablecer",
    legPick: "¿Qué partido quieres registrar?",
  },
  nav: {
    tournaments: "Torneos",
    teams: "Equipos",
    players: "Jugadores",
    history: "Historia",
    settings: "Configuración",
  },
  guide: {
    title: "Cómo usar",
    intro: "¿Eres nuevo aquí? Sigue estos pasos para crear y simular tu primer torneo.",
    steps: {
      teams: {
        title: "Añade tus equipos",
        desc: "Abre la pestaña Equipos y toca “+ Añadir equipo” para configurar un nombre, abreviatura, nivel de fuerza y color. ¿Tienes prisa? Ve a Configuración → Datos de muestra para cargar una lista de equipos predefinida.",
      },
      players: {
        title: "Añade jugadores (opcional)",
        desc: "Abre la página de un equipo para formar su plantilla — nombre, posición y potencia de cada jugador. Activa “Usar potencia de jugadores” en Configuración → Simulación para que la plantilla influya en los resultados.",
      },
      create: {
        title: "Crea un torneo",
        desc: "In la pestaña Torneos, toca “+ Nuevo torneo”, dale un nombre y elige los equipos. Selecciona un formato — Eliminatoria directa, Grupos + Eliminatoria, o Liga — luego define opciones como el número de grupos, partidos por eliminatoria (ida sola o ida y vuelta) y puntos por victoria/empate.",
      },
      play: {
        title: "Juega o simula partidos",
        desc: "Introduce los resultados tú mismo con “Definir resultado”, o pulsa “Simular todo” para jugarlo todo al instante. Explora las pestañas de Grupos, Eliminatorias, Liga, Estadísticas y Calendario, y usa “Avanzar a eliminatorias” una vez terminada la fase de grupos.",
      },
      simulate: {
        title: "Ejecuta una simulación de Montecarlo",
        desc: "Abre la Configuración de un torneo y toca “Simular 10.000×” para reproducirlo miles de veces. La página de Resultados de simulación muestra las probabilidades de campeonato de cada equipo y sus estadísticas promedio.",
      },
      seasons: {
        title: "Inicia nuevas temporadas y sigue el historial",
        desc: "Toca “Nueva temporada” para volver a jugar un torneo con los mismos equipos. La pestaña Historial reúne tus Campeones, la Clasificación histórica y las Estadísticas de todas las temporadas.",
      },
      settings: {
        title: "Personaliza en Configuración",
        desc: "Cambia el idioma, cambia de tema (Claro, Oscuro o Copa del Mundo 2026), ajusta las reglas de la tabla y las opciones de visualización, y haz una copia de seguridad o exporta todos tus datos desde la pestaña Configuración.",
      },
    },
    outro:
      "Eso es todo — puedes cambiar casi cualquier cosa más tarde desde la página de Configuración de un torneo.",
  },
  common: {
    next: "Siguiente",
    previousPage: "Página anterior",
    nextPage: "Página siguiente",
    pageOf: "{page}/{total}",
    remove: "Quitar",
    simulate: "Simular",
    info: "Información",
    pickColor: "Abrir selector de color",
    back: "Volver",
    close: "Cerrar",
    cancel: "Cancelar",
    optional: "Opcional",
    save: "Guardar",
    delete: "Eliminar",
    edit: "Editar",
    open: "Abrir",
    export: "Exportar",
    import: "Importar",
    reset: "Restablecer",
    load: "Cargar",
    on: "Sí",
    off: "No",
    show: "Mostrar",
    hide: "Ocultar",
    single: "Individual",
    double: "Doble",
    triple: "3×",
    quadruple: "4×",
    half: "0.5×",
    random: "Aleatorio",
    seeded: "Sembrado",
    manual: "Manual",
    search: "Buscar",
    filter: "Filtrar",
    create: "Crear",
    new: "Nuevo",
    noMatch: 'No hay resultados para "{query}".',
    season: "temporada | temporadas",
    teams: "{n} equipos",
    team: "Equipo",
    update: "Actualizar",
    newVersionAvailable: "Nueva versión disponible",
    starOnGithub: "Estrella en GitHub",
    star: "Estrella",
    setResult: "Ingresar resultado",
    moveUp: "Mover arriba",
    moveDown: "Mover abajo",
    vs: "vs",
    share: "Compartir",
    or: "o",
    offline: "Sin conexión",
    retry: "Reintentar",
    errorBoundaryMsg: "Algo salió mal. Por favor, inténtalo de nuevo.",
  },
  tournaments: {
    title: "Torneos",
    newBtn: "Nuevo torneo",
    needTeamsTitle: "Añade al menos 2 equipos primero",
    needTeamsNotice: "Añade al menos 2 equipos en la pestaña Equipos primero.",
    searchPlaceholder: "Buscar torneos…",
    viewList: "Vista de lista",
    viewGrid: "Vista de cuadrícula",
    empty: "Aún no hay torneos. Haz clic en {action} para empezar.",
    noMatch: 'No hay torneos que coincidan con "{query}".',
    live: "En curso",
    notFound: "Torneo no encontrado.",
    format: {
      groupsKo: "Grupos+KO",
      groupsKoLong: "Grupos + KO",
      swiss: "Suizo",
      league: "Liga",
      bracket: "Eliminatoria",
      custom: "Personalizado",
    },
    deleteConfirm: "¿Eliminar este torneo?",
    deleteLabel: "Eliminar",
    selectDatasetBtn: "Seleccionar conjunto de datos",
  },
  teams: {
    deleteInUse: "Este equipo no se puede eliminar porque se usa en un torneo.",
    title: "Equipos",
    notFound: "Equipo no encontrado.",
    addBtn: "Añadir equipo",
    loadDatasetTitle: "Cargar conjunto de datos",
    searchPlaceholder: "Buscar equipos…",
    empty: "Aún no hay equipos. Haz clic en {action} para empezar.",
    noMatch: 'No hay equipos que coincidan con "{query}".',
    limitReached: "Límite de equipos alcanzado ({max})",
    sortDefault: "Predeterminado",
    sortAsc: "Ascendente",
    sortDesc: "Descendente",
    form: {
      addTitle: "Añadir equipo",
      editTitle: "Editar equipo",
      name: "Nombre",
      abbreviation: "Abreviatura",
      power: "Poder",
      color: "Color",
      flag: "Bandera",
      flagNone: "Seleccionar bandera…",
      flagPickerTitle: "Selecciona una bandera",
      flagSearch: "Buscar país…",
      flagRemove: "Quitar bandera",
      imageSourceTitle: "Elegir imagen",
      imageSourceFlag: "Seleccionar bandera",
      imageSourceUrl: "Añadir URL de imagen",
      imageSourceGallery: "Elegir de la galería",
      imageUrlTitle: "Añadir URL de imagen",
      imageUrlPlaceholder: "https://…",
      imageRemove: "Quitar imagen",
      imageInvalid: "No se pudo cargar la imagen",
      namePlaceholder: "Nombre del equipo",
      randomName: "Nombre aleatorio",
    },
    detail: {
      tabs: {
        overview: "Resumen",
        matches: "Partidos",
      },
      powerRating: "Índice de poder",
      allStats: "Todas las estadísticas",
      played: "Jugados",
      wins: "Victorias",
      draws: "Empates",
      losses: "Derrotas",
      winRate: "Porcentaje de victorias",
      goalsFor: "Goles a favor",
      goalsAgainst: "Goles en contra",
      titles: "Títulos",
      recentForm: "Forma reciente",
      last5: "últimos 5",
      seasonHistory: "Historial por temporada",
      tournamentTitles: "Títulos de torneo",
      matchHistory: "Historial de partidos",
      matchCount: "{n} partidos",
      allTournaments: "Todos los torneos",
      seasonN: "Temporada {n}",
      noMatchesPlayed: "Aún no se han jugado partidos.",
      noMatchesTournament: "No hay partidos para este torneo.",
    },
  },
  players: {
    title: "Jugadores",
    squadTitle: "Plantilla",
    emptyOnTeam: "Aún no hay jugadores en esta plantilla.",
    addBtn: "Añadir",
    searchPlaceholder: "Buscar jugadores…",
    empty: "Aún no hay jugadores. Haz clic en {action} para empezar.",
    emptyNoTeams: "Crea primero un equipo y luego añade sus jugadores.",
    needTeamFirst: "Crea primero un equipo",
    noMatch: "Ningún jugador coincide con los filtros.",
    deleteAllConfirm: "¿Eliminar todos los jugadores?",
    sortDefault: "Predeterminado",
    positions: {
      GK: "Portero",
      DEF: "Defensa",
      MID: "Centrocampista",
      FWD: "Delantero",
    },
    filter: {
      teamLabel: "Equipo",
      sortLabel: "Ordenar por",
      allTeams: "Todos los equipos",
      searchPlaceholder: "Buscar equipo…",
      noResults: "No se encontraron equipos",
    },
    form: {
      number: "Dorsal",
      numberPlaceholder: "—",
      numberPickerTitle: "Seleccionar dorsal",
      numberClear: "Sin número",
      addTitle: "Añadir jugador",
      editTitle: "Editar jugador",
      name: "Nombre",
      namePlaceholder: "Nombre del jugador",
      randomName: "Nombre aleatorio",
      team: "Equipo",
      teamPlaceholder: "Seleccionar equipo…",
      position: "Posición",
      power: "Potencia",
    },
    generate: {
      buttonLabel: "Generar jugadores",
      title: "Generar jugadores",
      hint: "Completa las posiciones que faltan en esta plantilla, hasta 18 jugadores.",
      team: "Equipo",
      willAdd: "Se añadirán {count} jugadores:",
      willAddMulti: "Se añadirán {count} jugadores repartidos en {teams} equipos:",
      squadFull: "Esta plantilla ya está al completo.",
      noTeamsBelow: "Ningún equipo está por debajo de este tamaño de plantilla.",
      fillAllTeams: "Completar todos los equipos incompletos",
      minSquadSize: "Plantillas por debajo de",
      minSquadSizeHint: "Cualquier equipo con menos jugadores que este número se completará.",
      firstNames: "Nombres",
      lastNames: "Apellidos",
      namesPlaceholder: "Un nombre por línea",
      loadFile: "Cargar archivo",
      removeFile: "Eliminar archivo",
      fileLoaded: "{file} — {count} nombres cargados",
      clear: "Eliminar",
      useDefault: "Usar predeterminado",
      generateBtn: "Generar",
    },
  },
  history: {
    players: {
      emptyTitle: "Sin registros de jugadores",
      emptyDesc: "Las temporadas anteriores a esta actualización no tienen datos de jugadores.",
      seasonsShort: "Tmp",
    },
    title: "Historia",
    empty: "Aún no hay torneos.",
    noCompletedSeasons: "Aún no hay temporadas completadas.",
    tabs: {
      players: "Jugadores",
      champions: "Campeones",
      allSeasons: "Todas las temporadas",
      allFinals: "Todas las finales",
      allTimeTable: "Tabla histórica",
      statistics: "Estadísticas",
      teams: "Equipos",
    },
    table: {
      season: "Temporada",
      champion: "Campeón",
      result: "Resultado",
      runnerUp: "Subcampeón",
      thirdPlace: "3.er lugar",
      team: "Equipo",
      titles: "Títulos",
      finals: "Finales",
      seasonsPlayed: "Temporadas jugadas",
      seasons: "Temporadas",
      matchesPlayed: "Partidos jugados",
      played: "Jugados",
      won: "Ganados",
      drawn: "Empatados",
      lost: "Perdidos",
      goalsFor: "Goles a favor",
      goalsAgainst: "Goles en contra",
      goalDiff: "Diferencia de goles",
      points: "Puntos",
      cleanSheets: "Porterías a cero",
    },
    stats: {
      completedSeasons: "Temporadas completadas",
      totalMatches: "Partidos totales",
      totalGoals: "Goles totales",
      goalsPerMatch: "Goles por partido",
      records: "Récords",
      biggestWin: "Mayor victoria",
      topScorer: "Máximo goleador",
      goals: "{n} goles",
      cleanSheets: "Porterías a cero",
      achievements: "Logros",
      firstChampion: "Primer campeón",
      season: "Temporada {n}",
      longestStreak: "Racha más larga",
      inARow: "{n} consecutivos",
      defending: "Defensor",
      consecutive: "{n} seguidos",
    },
  },
  settings: {
    title: "Configuración",
    language: {
      label: "Idioma",
      desc: "Idioma de visualización de la aplicación",
    },
    appearance: {
      design: {
        label: "Lenguaje de diseño",
        desc: "Suave es sereno y vidrioso; Vivo es más audaz, con superficies tintadas y esquinas más grandes. Tu color principal no cambia.",
        soft: "Suave",
        vivid: "Vivo",
      },
      title: "Apariencia",
      theme: {
        label: "Tema",
        desc: "Esquema de colores de toda la aplicación",
        light: "Claro",
        dark: "Oscuro",
      },
      primaryColor: {
        label: "Color primario",
        desc: "Color de acento de toda la aplicación",
      },
    },
    tableRules: {
      title: "Reglas de tabla predeterminadas",
      tiebreaker: {
        label: "Desempate",
        desc: "Qué separa a los equipos igualados en puntos — se aplica a la fase de grupos y las tablas de liga. {h2h} — resultados entre equipos empatados primero · {goalDiff} — diferencia de goles general primero",
        h2h: "Enfrentamiento directo",
        goalDiff: "Diferencia de goles",
        h2hShort: "D. directo",
        goalDiffShort: "Dif. goles",
      },
    },
    matchDefaults: {
      title: "Valores predeterminados de partidos",
      intro:
        "Configuración de ida/vuelta aplicada al crear un nuevo torneo. Puedes modificarla por torneo.",
      groupStage: {
        label: "Fase de grupos",
        desc: "{single} — un partido por par · {double} — local & visitante, marcador agregado",
      },
      knockoutRounds: {
        label: "Rondas eliminatorias",
        desc: "{single} — el ganador avanza · {double} — eliminatoria a doble partido, el marcador agregado decide",
      },
      final: {
        label: "Final",
        desc: "Se puede configurar por separado de otras rondas eliminatorias",
      },
    },
    newTournament: {
      title: "Valores predeterminados de nuevo torneo",
      intro:
        "Valores prellenados en el diálogo Crear torneo. Puedes cambiarlos allí en cualquier momento.",
      knockoutDraw: {
        label: "Sorteo eliminatorio",
        desc: "Cómo se ubican los equipos en un cuadro de solo eliminación",
      },
      groupDraw: {
        label: "Sorteo de fase de grupos",
        desc: "Cómo se distribuyen los equipos en los grupos",
      },
      playoffSeeding: {
        label: "Siembra del playoff",
        desc: "Cómo se emparejan los clasificados de grupo al hacer el sorteo del cuadro",
      },
      drawLegend: {
        draw: "{random} — por azar · {seeded} — los mejores equipos separados · {manual} — tú los colocas",
        playoff:
          "{cross} — A1 vs B2, B1 vs A2 · {noRematch} — evita rivales del mismo grupo en Ronda 1 · {random} — totalmente aleatorio",
        cross: "Cruzado",
        noRematch: "Sin revancha",
        drawOptions: "Opciones de sorteo:",
        playoffSeeding: "Siembra del playoff:",
      },
      scoring: {
        title: "Puntuación por defecto",
        desc: "Valores iniciales al crear un torneo de grupos/liga",
        winPoints: "Victoria",
        drawPoints: "Empate",
        lossPoints: "Derrota",
      },
    },
    graphics: {
      title: "Gráficos",
      bracketQuality: {
        label: "Calidad del cuadro",
        desc: "Alta — renderizado nítido en todos los niveles de zoom · Baja — activa la aceleración GPU, puede reducir la nitidez al hacer zoom",
        high: "Alta",
        low: "Baja",
      },
    },
    effects: {
      title: "Efectos de victoria",
    },
    display: {
      liveEvents: {
        label: "Eventos del partido en vivo",
        desc: "Elige qué tipos de evento aparecen en la cronología del partido en vivo",
      },
      title: "Visualización",
      teamAbbr: {
        label: "Abreviaturas de equipos",
        desc: "Código corto (p. ej. {example}) mostrado en los espacios del cuadro y diseños compactos",
      },
      bracketStyle: {
        label: "Estilo del cuadro",
        desc: "{doubleSided} — equipos en ambos lados del cuadro · {classic} — cuadro único de izquierda a derecha · {auto} — Doble cara para 17+ equipos eliminados, Clásico en caso contrario",
        doubleSided: "Doble cara",
        classic: "Clásico",
        auto: "Automático",
      },
      confetti: {
        label: "Confeti del campeón",
        desc: "Lanzar confeti del color del ganador cuando termina un torneo",
      },
      sound: {
        label: "Sonido del ganador",
        desc: "Reproducir un sonido cuando termina un torneo",
      },
      bracketHover: {
        label: "Resaltar ruta en el cuadro",
        desc: "Pasa el cursor sobre un equipo para resaltar su camino y atenuar los demás",
      },
      bracketConnectorColors: {
        label: "Colores de conectores",
        desc: "Colorea las líneas del cuadro con el color de cada equipo a medida que avanzan",
      },
      gradualReveal: {
        label: "Revelación gradual",
        desc: "Los resultados aparecen uno a uno con una breve pausa en lugar de todos a la vez",
      },
      autoAdvanceFixtureStage: {
        label: "Avance automático del calendario",
        desc: "Tras simular una etapa en la pestaña Calendario, pasar automáticamente a la siguiente",
      },
    },
    simulation: {
      redCardImpact: {
        label: "Impacto de la tarjeta roja",
        desc: "Una expulsión debilita al equipo el resto del partido y le deja una pequeña penalización en el siguiente.",
      },
      injuries: {
        label: "Lesiones",
        desc: "Una lesión puede dejar a un jugador fuera de los próximos partidos de su equipo, por una duración aleatoria.",
      },
      title: "Simulación",
      homeAdvantage: {
        label: "Ventaja local",
        desc: "Bono de fuerza al equipo local. {zero} = sin ventaja · {default} = predeterminado · {max} = ventaja local muy fuerte.",
        neutral: "Neutro",
        slight: "Leve",
        moderate: "Moderado",
        strong: "Fuerte",
        dominant: "Dominante",
      },
      surpriseFactor: {
        label: "Factor sorpresa",
        desc: "Cuánto pueden los equipos débiles vencer a los fuertes. {zero} = la fuerza decide todo · {max} = pura suerte.",
        predictable: "Predecible",
        pureLuck: "Pura suerte",
        mostlySkill: "Mayormente habilidad",
        upsetHeavy: "Muchas sorpresas",
        balanced: "Equilibrado",
      },
      formFactor: {
        label: "Factor forma",
        desc: "Ajusta dinámicamente la fuerza del equipo según los últimos 5 resultados. Un equipo en racha gana hasta {plus} de fuerza, uno perdiendo consecutivamente baja hasta {minus}.",
      },
      usePlayerPower: {
        label: "Usar potencia de jugadores",
        desc: "Combina la potencia de un equipo con la potencia media de los jugadores de su plantilla. Desactivado = solo se usa la potencia propia del equipo.",
      },
      fatigueFactor: {
        label: "Fatiga de los jugadores",
        desc: "Los jugadores se cansan a medida que juegan partidos y solo se recuperan parcialmente entre ellos. Un once titular cansado debilita a su equipo, recibe peores notas individuales y la IA lo alinea con menos frecuencia.",
      },
      moraleFactor: {
        label: "Moral del equipo",
        desc: "La racha actual de victorias o derrotas de un equipo influye en su potencia, al margen de su forma reciente.",
      },
      injuryFatigueImpact: {
        label: "La fatiga aumenta el riesgo de lesión",
        desc: "Un jugador cansado se lesiona con más facilidad. Requiere que las Lesiones estén activadas.",
      },
    },
    sampleData: {
      withSquads: "Plantillas",
      title: "Datos de muestra",
      countries: "Selecciones",
      clubs: "Clubes",
      uefa: "Competiciones europeas",
      leagues: "Ligas",
      classics: "Clásicos",
      intro:
        "Carga una lista de equipos preestablecida. Se añadirá junto a tus equipos y torneos actuales — nada existente se elimina.",
      loadConfirm:
        '¿Cargar el conjunto de datos "{name}"? Se añadirá a tus equipos y torneos actuales.',
      loadConfirmAd:
        '¿Cargar el conjunto de datos "{name}"? Se añadirá a tus equipos y torneos actuales. Cada 2 conjuntos de datos que cargues se muestra un breve anuncio con recompensa — este es uno de ellos.',
      adNotice:
        "La próxima vez que cargues un conjunto de datos se mostrará un breve anuncio con recompensa.",
      adBadge: "Ver anuncio",
      loadLabel: "Cargar",
    },
    dataManagement: {
      title: "Gestión de datos",
      backup: {
        label: "Copia de seguridad",
        desc: "Guarda todos tus equipos y torneos en un archivo, o restaura desde uno",
      },
      clearAll: {
        label: "Borrar todos los datos",
        desc: "Elimina permanentemente todos los equipos y torneos — no se puede deshacer",
        confirmMsg: "¿Estás seguro de que quieres borrar todos los datos? No se puede deshacer.",
        confirmLabel: "Borrar todo",
        btn: "Borrar todo",
      },
      invalidFile: "Archivo de copia de seguridad no válido.",
      partialImport: "Importado, pero no se pudieron leer {count} torneo(s) y se omitieron.",
    },
    resetSettings: {
      title: "Restablecer configuración",
      label: "Restaurar valores predeterminados",
      desc: "Restaura todos los ajustes a sus valores originales. Los equipos y torneos no se ven afectados.",
      btn: "Restablecer",
    },
    nav: {
      appearance: "Idioma y apariencia",
      bracket: "Cuadro y efectos",
      tournament: "Reglas del torneo",
      simulation: "Simulación",
      music: "Música",
      sampleData: "Datos de muestra",
      data: "Datos",
    },
    tutorials: "Tutoriales",
    about: "Acerca de",
    aboutBy: "por",
    specialThanks: "Agradecimiento especial",
    disclaimer: "Simulador no oficial hecho por aficionados. No está afiliado, respaldado ni conectado con la UEFA, la AFC, la CONMEBOL, la FIFA ni ningún club, liga o competición mencionados en la app.",
  },
  tournament: {
    exportExcel: "Exportar a Excel",
    noFixtures: "Todavía no hay partidos.",
    groupStageNotStarted: "La fase de grupos aún no ha comenzado.",
    matchdayProgress: "{played}/{total} jornadas",
    champion: "Campeón",
    simulateMatchday: "Simular jornada",
    wildcardRace: "Carrera por el comodín",
    aggregate: "Global",
    notFound: "Torneo no encontrado.",
    newSeason: "Nueva temporada",
    simulateAll: "Simular todo",
    simulate: "Simular",
    simulateRound: "Simular {round}",
    simulateThirdPlace: "Simular Partido por el 3.er Puesto",
    simulateWeek: "Simular Jornada",
    simulateGroup: "Simular {group}",
    advanceToKnockout: "Avanzar a la Fase Eliminatoria",
    settings: "Configuración",
    previousSeason: "Temporada anterior",
    nextSeason: "Temporada siguiente",
    fixtures: {
      groupWeek: "Semana {n}",
      simulateStage: "Simular esta etapa",
    },
    locked: {
      bracketNeedsGroups: "Termina la fase de grupos para desbloquear el cuadro eliminatorio.",
      bracketNeedsPlayoff: "Termina la temporada para desbloquear el playoff.",
      stats: "Juega algunos partidos para ver las estadísticas.",
    },
    tabs: {
      groups: "Grupos",
      bracket: "Cuadro",
      league: "Liga",
      stats: "Estadísticas",
      participants: "Participantes",
      wildcards: "Comodines",
      standings: "Clasificación",
      fixtures: "Calendario",
      playoff: "Playoff",
      swissPhase: "Fase de Liga",
    },
    header: {
      teams: "{n} equipos",
      created: "Creado el {date}",
    },
    newSeasonModal: {
      useOldDraw: "Usar el sorteo anterior",
      useOldDrawDesc: "Mantener el mismo orden del sorteo de la Temporada {n}",
      newDraw: "Nuevo sorteo",
      newDrawDesc: "Añade o elimina equipos y luego realiza el sorteo",
      currentTeams: "En el torneo",
      availableTeams: "Equipos disponibles",
      noAvailable: "No hay equipos disponibles",
      noTeams: "No hay equipos seleccionados",
    },
    create: {
      title: "Nuevo torneo",
      name: "Nombre del torneo",
      namePlaceholder: "p. ej. Campeonato de Primavera 2025",
      randomName: "Nombre aleatorio",
      teams: "Equipos participantes",
      selected: "({n} seleccionados)",
      selectAll: "Seleccionar todos",
      deselectAll: "Deseleccionar todos",
      minTeams: "Selecciona al menos 2 equipos.",
      format: "Formato del torneo",
      formats: {
        bracket: "Cuadro eliminatorio",
        bracketDesc: "Pierdes una vez y estás fuera",
        groupsKo: "Grupos + Eliminatorias",
        groupsKoDesc: "Fase de grupos, luego los mejores equipos avanzan",
        league: "Liga",
        leagueDesc: "Todos juegan contra todos, gana quien más puntos acumule",
        leagueKo: "Liga + Eliminatorias",
        leagueKoDesc: "Temporada regular y luego los mejores disputan un playoff",
        swiss: "Sistema Suizo",
        swissDesc:
          "Una sola tabla, pero cada equipo se enfrenta solo a parte del campo (al estilo de la Champions League)",
        custom: "Personalizado",
        customDesc:
          "Monta tus propias fases: grupos, tablas y eliminatorias, en el orden que quieras",
      },
      swissConfig: {
        phase: "Fase de Liga",
        opponents: "Rivales por equipo",
        opponentsHint: "Cuántos equipos distintos enfrenta cada uno",
        legs: "Partidos por rival",
        homeAwayBalance: "Equilibrar local y visitante",
        draw: "Sorteo",
        drawType: "Método",
        drawSeeded: "Por fuerza (bombos)",
        drawRandom: "Aleatorio",
        pots: "Número de bombos",
        potsHint: "{size} equipos por bombo · {perPot} rivales de cada uno",
        potsNone: "Un solo bombo — sin cabezas de serie",
        summary: "{matchdays} jornadas · {matches} partidos",
        lockedBanner: "Bloqueado una vez introducidos los resultados",
        errors: {
          minTeams: "Se necesitan al menos 4 equipos",
          minOpponents: "Se necesita al menos 1 rival",
          tooManyOpponents: "Un equipo no puede enfrentar a más rivales que equipos hay",
          oddProduct: "Equipos × rivales debe ser un número par",
          potSizeUneven: "Los bombos deben dividir los equipos por igual",
          potDivision: "Los rivales deben repartirse por igual entre los bombos",
          oddPotQuota: "Tamaño del bombo × rivales por bombo debe ser par",
        },
      },
      config: {
        group: "Configuración de Grupos",
        knockout: "Configuración de Eliminatorias",
        league: "Configuración de Liga",
        groupsAndAdvance: "{groups} grupos · {advance} avanzan",
        wildcardsShort: "+{n} comodín",
        thirdPlaceOn: "partido por el 3er puesto",
        noThirdPlace: "sin 3er puesto",
        legsShort: "Eliminatorias {ko} · Final {final}",
        divisionsShort: "{n} divisiones",
        singleDivisionShort: "división única",
        playoffShort: "playoff de {n} equipos",
        playoffOff: "Sin fase de playoff",
        swiss: "Configuración Suiza",
        swissOpponentsShort: "{n} rivales",
        swissPotsShort: "{n} bombos",
      },
      groups: "Número de grupos",
      groupSizeEven: "{count} grupos de {size} equipos",
      groupSizeSplit: "{countA} grupos de {sizeA} equipos · {countB} grupos de {sizeB} equipos",
      advance: "Equipos que avanzan",
      perGroup: "por grupo",
      reachKnockout: "llegan a eliminatorias",
      wildcards: "Mejores subcampeones comodín",
      total: "total",
      roundFormat: "Formato de ronda",
      schedule: "Calendario",
      leagueHint:
        "{single} — una vez · {double} — local & visitante · {triple} — 3 partidos · {quad} — 4 partidos (2L & 2V)",
      tiers: "Divisiones de liga",
      numberOfTiers: "Número de divisiones",
      singleDivision: "una sola división",
      divisions: "{n} divisiones",
      promotionRelegation: "Ascenso / Descenso",
      teamsSwap: "los equipos intercambian entre divisiones adyacentes",
      playoff: {
        enable: "Añadir una fase de playoff",
        hint: "Los mejores equipos clasifican a un playoff eliminatorio tras la temporada regular.",
        qualifierCount: "Equipos en el playoff",
        qualifierCountHint: "mejores equipos de la tabla final que llegan al playoff",
        seedMode: "Emparejamiento",
      },
      drawMethod: "Método de sorteo",
      groupStage: "Fase de grupos",
      playoffBracket: "Cuadro de playoff",
      drawHint:
        "{random} — por azar · {seeded} — los mejores separados · {manual} — tú los organizas",
      playoffHint:
        "{cross} — A1 vs B2, B1 vs A2 · {noRematch} — evita rivales del mismo grupo en Ronda 1 · {random} — totalmente aleatorio",
      cross: "Cruzado",
      noRematch: "Sin revancha",
      options: "Opciones",
      thirdPlace: "Partido por el 3.er puesto",
      thirdPlaceHint: "Los perdedores de semifinales juegan por la medalla de bronce",
      tiebreaker: "Desempate",
      createBtn: "Crear torneo",
    },
    setResult: "Ingresar resultado",
    simulationResults: {
      noResults: "No se encontraron resultados de simulación.",
      runFirst: "Ejecuta una simulación primero",
      title: "Resultados de simulación",
      runs: "{n} ejecuciones",
      season: "Temporada {n}",
      format: "Formato",
      teams: "Equipos",
      simulations: "Simulaciones",
      championshipOdds: "Probabilidades de campeonato",
      leagueOdds: "Probabilidades de liga",
      winProbability: "Probabilidad de victoria",
      titleProbability: "Probabilidad de título",
      win: "Victoria",
      final: "Final",
      top4: "Top 4",
      adv: "Av.",
      titleCol: "Título",
      top3: "Top 3",
      avgPts: "Pts prom.",
      avgGF: "GF prom.",
      avgGA: "GA prom.",
      saveJson: "Guardar JSON",
      saveCsv: "Guardar CSV",
    },
    settingsPage: {
      title: "Configuración del torneo",
      notFound: "Torneo no encontrado.",
      backToTournaments: "Volver a torneos",
      locked: "Bloqueado",
      saveChanges: "Guardar cambios",
      tournamentName: {
        title: "Nombre del torneo",
        hint: "Renombrar actualizará las {n} temporadas que comparten este nombre.",
      },
      info: {
        created: "Creado el {date}",
        teams: "{n} equipos",
        champion: "Campeón",
      },
      manageTeams: {
        title: "Gestionar equipos",
        addPlaceholder: "Añadir equipo…",
        add: "Añadir",
        allAdded: "Todos los equipos disponibles ya están en este torneo.",
        lockedBanner: "La gestión de equipos se deshabilita una vez que se han jugado partidos.",
      },
      drawMethod: {
        lockedBanner: "El sorteo no se puede cambiar una vez que se han jugado partidos.",
        regenerate: "↺ Regenerar",
        redrawConfirm: "¿Restablecer el sorteo y regenerar?",
        redrawConfirmLabel: "Regenerar",
      },
      groupStructure: {
        title: "Estructura de grupos",
        lockedBanner:
          "La estructura de grupos no se puede cambiar una vez que se han jugado partidos.",
      },
      qualification: {
        title: "Clasificación y comodines",
        teamsAdvance: "Equipos que avanzan por grupo",
        reachKnockout: "→ {n} llegan a la fase eliminatoria",
        total: "→ {n} total",
        lockedBanner: "Bloqueado — el cuadro eliminatorio ya se generó a partir de los grupos.",
      },
      playoffSeeding: {
        title: "Cabezas de serie del playoff",
        lockedBanner: "Bloqueado — la fase de grupos está completa y el cuadro ha sido generado.",
      },
      formatOptions: {
        title: "Opciones de formato",
        lockedBanner: "El formato no se puede cambiar una vez que se han jugado partidos.",
      },
      legsPerMatch: {
        title: "Partidos por ronda",
        hintSingle: "1 partido, el ganador avanza",
        hintDouble: "ida y vuelta, goles agregados deciden",
        r64: "Treintaidosavos de Final",
        r32: "Dieciseisavos de Final",
        r16: "Octavos de Final",
        quarterFinal: "Cuartos de Final",
        semiFinal: "Semifinal",
        final: "Final",
        thirdPlace: "Tercer y cuarto puesto",
        lockedBanner:
          "La configuración de partidos no se puede cambiar tras el inicio de los partidos.",
      },
      leagueFormat: {
        title: "Formato de liga",
        hintSingle: "una vez",
        hintDouble: "ida y vuelta",
        hintTriple: "3 encuentros",
        hintQuad: "4 encuentros (2L & 2V)",
        lockedBanner: "El formato no se puede cambiar una vez iniciados los partidos.",
        numberOfDivisions: "Número de divisiones",
        minTeams: "mín. 2 equipos por división",
        slotsSwapped: "plazas intercambiadas entre divisiones adyacentes",
        divisions: {
          title: "Divisiones",
          lockedBanner: "Las divisiones no se pueden cambiar una vez iniciados los partidos.",
        },
        playoff: {
          title: "Playoff",
          lockedBanner: "Los ajustes del playoff no se pueden cambiar una vez iniciado.",
        },
      },
      tiebreaker: {
        method: "Método",
        h2h: "H2H",
        goalDiff: "Diferencia de goles",
      },
      scoring: {
        title: "Puntuación",
        winPoints: "Puntos por victoria",
        drawPoints: "Puntos por empate",
        lossPoints: "Puntos por derrota",
        lockedBanner: "La puntuación no se puede cambiar una vez que se han jugado partidos.",
      },
      simulation: {
        title: "Simulación Monte Carlo",
        desc: "Simula este torneo 10.000 veces y obtén probabilidades de victoria para cada equipo.",
        run: "Simular 10.000×",
        simulating: "Simulando {name}…",
        done: "Simulación completa — 10.000 ejecuciones finalizadas.",
        viewResults: "Ver resultados",
      },
      teamAdjustments: {
        title: "Ajustes de equipos",
        pointsTitle: "Ajuste de puntos de equipos",
        powerTitle: "Ajuste de poder de equipos",
        pointsHint:
          "Añade o resta puntos en la tabla para equipos específicos esta temporada. Se reinicia en la próxima temporada.",
        powerHint:
          "Aumenta o reduce temporalmente la potencia de un equipo para la simulación esta temporada. Se reinicia en la próxima temporada.",
        lockedBanner: "Los ajustes no se pueden cambiar una vez que los partidos han comenzado.",
      },
      dangerZone: {
        title: "Zona de peligro",
        resetLabel: "Restablecer torneo",
        resetDesc: "Borra todos los resultados de los partidos, manteniendo equipos y sorteo.",
        resetConfirm: "¿Restablecer este torneo?",
        resetConfirmLabel: "Restablecer",
        deleteLabel: "Eliminar torneo",
        deleteDesc: "Elimina permanentemente este torneo y todos sus datos.",
        deleteConfirm: "¿Eliminar este torneo?",
        deleteConfirmLabel: "Eliminar",
      },
      unsavedChanges: {
        title: "Cambios sin guardar",
        message: "Tienes cambios sin guardar. ¿Qué deseas hacer?",
        leave: "Salir",
        saveAndLeave: "Guardar y salir",
        close: "Cerrar",
      },
    },
    phases: {
      title: "Fases",
      help: "Arrastra el puerto derecho de una fase hasta el puerto izquierdo de la siguiente.",
      summary: "{count} fases",
      summaryEmpty: "Aún no hay fases",
      button: "Fases",
      kinds: {
        group: "Grupos",
        league: "Liga",
        swiss: "Suizo",
        knockout: "Eliminatoria",
      },
      node: {
        name: "Nombre de la fase",
        teams: "{count} equipos",
        config: "Ajustes",
        final: "Final",
        remove: "Eliminar fase",
      },
      guide: {
        lead: "Construye el torneo con fases y conéctalas entre sí.",
        add: "Añade una fase con los botones de arriba: una fase de grupos, una tabla, una ronda suiza o una eliminatoria.",
        connect:
          "Arrastra desde el puerto derecho de una fase hasta el puerto izquierdo de la siguiente. En una pantalla táctil, toca un puerto y luego el otro.",
        configure:
          "Abre los Ajustes de una fase para definir su formato, y su conexión para elegir qué puestos avanzan.",
        final:
          "Marca como Final la fase que decide el campeón. Una eliminatoria no tiene puerto de salida: ahí termina una rama.",
        example:
          "Una estructura habitual: Grupos → Liga → Copa. Los grupos envían sus clasificados a la liga, los cuatro primeros de la liga pasan a la copa, y la copa es la final.",
        dismiss: "Entendido",
        reopen: "¿Cómo funciona esto?",
      },
      links: {
        title: "Conexiones",
        edit: "Cambiar qué puestos avanzan",
        remove: "Eliminar conexión",
      },
      edge: {
        single: "Puesto {rank}",
        range: "Puestos {from}-{to}",
        teams: "{count} equipos",
      },
      range: {
        title: "Quién avanza",
        subtitle: "{from} a {to}",
        from: "Desde el puesto",
        to: "Hasta el puesto",
        hint: "{count} equipos pasan a {to}.",
        disconnect: "Desconectar",
        groupNote:
          "{from} decide quién avanza en sus propios ajustes: clasificados por grupo, más los comodines.",
      },
      config: {
        intake: "{count} equipos llegan a esta fase",
        shape: "Formato",
        groupCount: "Número de grupos",
        seedMode: "Sorteo",
        legs: "Partidos por enfrentamiento",
        opponents: "Rivales por equipo",
        pots: "Número de bombos",
        balance: "Equilibrar local y visitante",
        finalLegs: "Final",
        thirdPlace: "Partido por el tercer puesto",
        thirdPlaceLegs: "Partidos por el tercer puesto",
        swissSize: "{matchdays} jornadas",
        qualification: "Clasificación",
        qualifiersPerGroup: "Avanzan por grupo",
        wildcards: "Comodines",
        wildcardsHint: "Los mejores equipos que terminan un puesto por debajo",
        advancing: "{count} equipos avanzan",
      },
      errors: {
        noPhases: "Añade al menos una fase.",
        noEntry: "Una fase no debe tener ninguna conexión entrante: por ahí entran los equipos.",
        multipleEntry: "Solo una fase puede ser la fase inicial.",
        cycle: "Las conexiones forman un bucle.",
        unnamed: "Dale un nombre a esta fase.",
        knockoutHasOutput: "Una eliminatoria no puede enviar equipos más adelante.",
        noFinal: "Marca una fase como final.",
        multipleFinal: "Solo una fase puede ser la final.",
        finalNotTerminal: "La fase final no puede enviar equipos más adelante.",
        edgeRankRange: "Los puestos {from}-{to} no existen: esta fase tiene {available} equipos.",
        rangeOverlap: "Dos conexiones reclaman los mismos puestos.",
        phaseTooSmall: "{intake} equipos son muy pocos: esta fase necesita al menos {min}.",
        groupCount: "Establece al menos dos grupos.",
        groupsTooSmall: "{intake} equipos no pueden llenar {groupCount} grupos.",
        groupSingleOutput:
          "Una fase de grupos envía a sus clasificados como un solo conjunto: solo puede tener una conexión.",
        groupQualifiers: "{qualifiers} por grupo son demasiados: como máximo {max}.",
        groupWildcards: "{wildcards} comodines son demasiados: como máximo {max}, uno por grupo.",
        swissOpponents: "{opponents} rivales son demasiados: como máximo {max}.",
        configMismatch: "Los ajustes de esta fase no coinciden con su tipo.",
        danglingEdge: "Una conexión apunta a una fase que ya no existe.",
        selfEdge: "Una fase no puede conectarse consigo misma.",
      },
      advance: "Avanzar a {phase}",
      locked: "Termina {phase} para desbloquear esta etapa.",
      notStarted: "Esta fase aún no ha comenzado.",
    },
  },
  teamSelector: {
    searchPlaceholder: "Buscar equipos…",
    selected: "Seleccionados",
    available: "Disponibles",
    selectAll: "Seleccionar todo",
    deselectAll: "Deseleccionar todo",
    emptySelected: "Sin equipos seleccionados",
    emptyAvailable: "No se encontraron equipos",
    minTeams: "Selecciona al menos 2 equipos",
    sortName: "Nombre",
    sortPower: "Fuerza",
    title: "Equipos",
    fullView: "Vista completa",
  },
  manualDraw: {
    byesTitle: "Byes (victoria automática)",
    bye: "BYE",
    matches: "Partidos",
    home: "Local",
    away: "Visitante",
    confirmDraw: "Confirmar sorteo",
    groupDrawHint:
      "Arrastra equipos a cada grupo, o toca un equipo y luego una casilla para asignarlo.",
    groupLabel: "Grupo {name}",
    poolTitle: "Equipos disponibles",
    progress: "{done}/{total} asignados",
    dropHere: "Suelta o toca el equipo aquí",
    wildcard: "Comodín",
    remove: "Quitar",
  },
  leaguePlayoff: {
    startPlayoff: "Iniciar Playoff",
    finishSeasonFirst: "Termina la temporada para iniciar el playoff.",
    qualifier: "Puesto {rank}",
    playoffDrawTitle: "Sorteo de Playoff",
    potTop: "Cabezas de serie",
    potRest: "Resto",
  },
  coach: {
    title: "Entrenador",
    appoint: "Nombrar",
    empty: "Sin entrenador. El equipo juega sin ninguna instrucción particular.",
    styles: {
      attacking: "Ofensivo",
      balanced: "Equilibrado",
      defensive: "Defensivo",
    },
    styleHints: {
      attacking: "Marca más, encaja más.",
      balanced: "Sin instrucciones en ningún sentido.",
      defensive: "Encaja menos, marca menos.",
    },
    form: {
      title: "Entrenador",
      hint: "La formación, el estilo y la valoración del propio entrenador cambian cómo se desarrollan los partidos de este equipo.",
      name: "Nombre",
      namePlaceholder: "Nombre del entrenador",
      formation: "Formación",
      formationHint: "La disposición con la que sale el equipo.",
      style: "Estilo de juego",
      power: "Valoración del entrenador",
      randomize: "Aleatorio",
      sack: "Quitar",
    },
    generate: {
      title: "Generar entrenadores",
      buttonLabel: "Generar entrenadores",
      hint: "Asigna un entrenador a cada equipo, con una valoración cercana a la fuerza del club.",
      overwrite: "Reemplazar entrenadores existentes",
      willAdd: "Se nombrarán {count} entrenador(es)",
      keeping: "{count} conservados",
      allCoached: "Todos los equipos ya tienen entrenador",
      generateBtn: "Generar",
    },
  },
  manager: {
    match: {
      title: "Entrenador",
      manage: "Gestionar",
      onPitch: "En el campo",
    },
    tactics: {
      title: "Táctica",
      hint: "Se aplica desde el próximo minuto. Lo ya jugado no cambia.",
      apply: "Aplicar",
    },
    sub: {
      title: "Sustitución",
      pickOut: "¿Quién sale?",
      pickIn: "¿Quién entra?",
      back: "Atrás",
      left: "Quedan {count}",
      noneLeft: "No quedan sustituciones",
      emptyBench: "Nadie en el banquillo",
      unknownPlayer: "Jugador desconocido",
      failed: "No se pudo hacer esa sustitución",
    },
    banner: {
      title: "{team} — tu próximo partido",
      play: "Gestionar",
    },
    settings: {
      title: "Modo Entrenador",
      team: "Dirigir un equipo",
      hint: "Juegas tú mismo todos los partidos de este equipo, toda la temporada. La simulación masiva espera por ti.",
      placeholder: "Nadie — simular todo",
      standDown: "Dejar el cargo",
      blocked: "Juega primero tu propio partido",
    },
    panel: {
      allPlayed: "No te queda nada por jugar — el resto se resuelve solo.",
    },
    lineup: {
      title: "Once inicial",
      count: "{n}/11",
      hint: "Elige tu once por posición — el partido no arrancará hasta que se ocupen todas las camisetas.",
      noSquad: "No hay jugadores registrados en este equipo.",
      noneForPosition: "No hay jugadores en esta posición.",
      incomplete: "Te faltan {n} por elegir antes de poder empezar.",
      injured: "Lesionado",
      suspended: "Sancionado",
      fatigueHint:
        "El distintivo de la camiseta es la resistencia del jugador: verde, fresco; ámbar, cansado; rojo, agotado. 100 % significa totalmente fresco; baja hacia 0 % a medida que se cansa.",
      pickTitle: "Elegir jugador",
      clear: "Quitar de la alineación",
      outOfPosition: "Fuera de posición",
      searchPlaceholder: "Buscar jugadores…",
      fatigue: {
        fresh: "Fresco",
        tired: "Cansado",
        exhausted: "Agotado",
      },
    },
    morale: {
      label: "Moral",
    },
  },
  music: {
    title: "Música de fondo",
    builtIn: "Integrada",
    tracksTitle: "Pistas",
    tracksHint:
      "Añade tus propios archivos de audio, hasta {max} MB cada uno. Se guardan solo en este dispositivo.",
    addTrack: "Añadir pista",
    manage: "Gestionar",
    manageDesc: "Añade o elimina tus propias pistas",
    invalidType: "Eso no es un archivo de audio.",
    tooLarge: "Ese archivo supera los {max} MB.",
    saveFailed: "No se pudo guardar la pista.",
    enabled: {
      label: "Reproducir música",
      desc: "Un bucle tranquilo mientras juegas. Activado por defecto.",
    },
    volume: {
      label: "Volumen",
      desc: "Qué tan alto suena la música bajo la app",
      silent: "Silencio",
      quiet: "Bajo",
      gentle: "Suave",
      loud: "Alto",
    },
    track: {
      label: "Pista",
      desc: "Lo que suena mientras la app está abierta",
    },
    loop: {
      label: "Repetir una pista",
      desc: "Desactivado reproduce toda la lista en orden",
    },
  },
  liveMatch: {
    title: "En vivo",
    replayTitle: "Repetición",
    watch: "En vivo",
    replay: "Repetir",
    skip: "Saltar",
    pause: "Pausa",
    resume: "Reanudar",
    kickOff: "Saque inicial",
    halfTime: "Descanso",
    fullTime: "Final del partido",
    extraTime: "Prórroga",
    extraTimeShort: "PRÓ",
    penalties: "Penaltis",
    speedOption: "{value}x",
    useResult: "Usar resultado",
    settingsLabel: "Velocidad del partido en vivo",
    settingsDesc: "A qué velocidad se juega un partido cuando lo ves en vivo",
  },
  matchStats: {
    aetFull: "Tras la prórroga · {home}–{away} al minuto 90",
    aet: "pró.",
    shootoutShort: "Penaltis",
    shootoutTitle: "Tanda de penaltis",
    manOfTheMatch: "Jugador del partido",
    title: "Informe del partido",
    buttonLabel: "Informe",
    comparison: "Estadísticas del partido",
    timeline: "Cronología",
    lineups: "Puntuaciones",
    possession: "Posesión",
    shots: "Tiros",
    onTarget: "A puerta",
    xg: "xG",
    bigChances: "Ocasiones claras",
    corners: "Córners",
    offsides: "Fueras de juego",
    fouls: "Faltas",
    shootout: "Penaltis {home}–{away}",
    unknownPlayer: "Jugador desconocido",
    assistBy: "Asistencia: {name}",
    noEvents: "Sin eventos en este partido.",
    legacyNotice: "Este partido se jugó antes de que existieran los informes.",
    player: "Jugador",
    goalsShort: "G",
    goalsFull: "Goles",
    assistsShort: "A",
    assistsFull: "Asistencias",
    ratingShort: "Nota",
    home: "Local",
    away: "Visitante",
    events: {
      goal: "Gol",
      penGoal: "Penalti",
      ownGoal: "Gol en propia puerta",
      penMiss: "Penalti fallado",
      yellow: "Tarjeta amarilla",
      red: "Tarjeta roja",
      sub: "Sustitución",
      subFor: "Entra por {name}",
      penShort: "(P)",
      ownShort: "(PP)",
    },
  },
  playerStats: {
    scorers: "Goleadores",
    assists: "Asistencias",
    rated: "Notas",
    keepers: "Porteros",
    player: "Jugador",
    colGoals: "G",
    colAssists: "A",
    colRating: "Nota",
    colCleanSheets: "PI",
    colConceded: "GC",
    colSaves: "Par",
    colApps: "PJ",
    emptyTitle: "Sin estadísticas de jugadores",
    emptyDesc: "Juega un partido para ver quién marcó.",
    emptyHint: "Todavía no hay nada que clasificar.",
    emptyRated: "Las notas aparecen tras {count} partidos.",
  },
  playerDetail: {
    yellowCards: "Amarillas",
    redCards: "Rojas",
    injuries: "Lesiones",
    matchesMissedToInjury: "Bajas por lesión",
    notFound: "Jugador no encontrado.",
    careerTitle: "Carrera",
    honoursTitle: "Palmarés",
    apps: "Partidos",
    goals: "Goles",
    assists: "Asistencias",
    saves: "Paradas",
    conceded: "Goles encajados",
    cleanSheets: "Porterías a cero",
    avgRating: "Nota media",
    bestRating: "Mejor nota",
    honourTitle: "Campeón",
    honourTopScorer: "Máximo goleador — {goals} goles",
    emptyTitle: "Sin partidos",
    emptyDesc: "Las estadísticas aparecerán cuando salte al campo.",
  },
  stats: {
    teamStats: "Estadísticas de equipos",
    playerStats: "Estadísticas de jugadores",
    round: "Ronda {round}",
    league: "Liga",
    group: "Grupo",
    standingsProgress: "{name} — Progreso en la tabla",
    defaultStandingsProgress: "Progreso en la tabla",
    topScorers: "Máximos Goleadores",
    bestDefense: "Mejor Defensa",
    team: "Equipo",
    goalsForTitle: "Goles a Favor",
    gf: "GF",
    goalsAgainstTitle: "Goles en Contra",
    ga: "GC",
    matchesPlayedTitle: "Partidos Jugados",
    mp: "PJ",
  },
  progressChart: {
    title: "Progreso de Puntos / Posición",
    position: "Posición",
    points: "Puntos",
    pts: "pts",
  },
  legend: {
    qualifies: "Clasificado",
    wildcard: "Wildcard (mejores {count})",
    eliminated: "Eliminado",
  },
  participants: {
    rank: "#",
    team: "Equipo",
    group: "Grupo",
    wins: "G",
    draws: "E",
    losses: "P",
    gf: "GF",
    ga: "GC",
    gd: "DG",
    power: "POD",
    result: "Resultado",
    place: "{ordinal} Lugar",
    eliminated: "Eliminado · {round}",
  },
  rounds: {
    matchday: "Jornada {n}",
    final: "Final",
    semiFinals: "Semifinales",
    quarterFinals: "Cuartos de final",
    roundOf: "Ronda de {n}",
    thirdPlace: "Partido por el tercer puesto",
  },
  notFound: {
    message: "Página no encontrada",
    goHome: "Ir al inicio",
  },
} as const
